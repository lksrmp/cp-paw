...
CHARACTER(16), ALLOCATABLE :: ATOMNAMES(:)  ! ATOM IDENTIFIERS OF ATOMS IN RIXS CALCULATION
INTEGER(4), ALLOCATABLE :: ISPECIES(:)  ! INTERNAL INDEX OF SPECIES OF ATOMS IN RIXS CALCULATION
...
INTEGER(4), ALLOCATABLE :: IATMAP(:)  ! MAPS TO ATOMS OF RIXS CALCULATION
INTEGER(4), ALLOCATABLE :: INDMAP(:)  ! MAPS TO FIRST PROJECTOR INDEX OF ATOMS
...
! DIPOLE MATRIX ELEMENTS <PHI_VALENCE|D(EPSILON)|PHI_CORE>
! DMATI FOR INCOMING POLARISATION, DMATF FOR OUTGOING POLARISATION
COMPLEX(8), ALLOCATABLE :: DPMATI(:,:,:,:)  ! DPMATI(2LCORE+1,2LVAL+1,VNPRO,NSPECTRA) DIPOLE MATRIX ELEMENTS
COMPLEX(8), ALLOCATABLE :: DPMATF(:,:,:,:)  ! DPMATF(2LCORE+1,2LVAL+1,VNPRO,NSPECTRA) DIPOLE MATRIX ELEMENTS
LOGICAL(4) :: TINIT=.FALSE.
CONTAINS
  SUBROUTINE DATA_DEALLOC
  IMPLICIT NONE
  IF(ALLOCATED(ATOMNAMES)) DEALLOCATE(ATOMNAMES)
  IF(ALLOCATED(ISPECIES)) DEALLOCATE(ISPECIES)
  ...
  IF(ALLOCATED(TKINV)) DEALLOCATE(TKINV)
  IF(ALLOCATED(DPMATI)) DEALLOCATE(DPMATI)
  IF(ALLOCATED(DPMATF)) DEALLOCATE(DPMATF)
...
INTEGER(4) :: NCORE
INTEGER(4) :: LCORE
! TODO: CHECK IF NVAL IS NECESSARY
INTEGER(4) :: NVAL
INTEGER(4) :: LVAL
...
! WARNING: MAKE SURE Q AND R ARE COMPATIBLE, BOTH EITHER ABSOLUTE OR RELATIVE
      ...
      USE DATA_MODULE, ONLY: NATOMS,INDMAP,ISPECIES,INDTYPE,RXPOS,DPMATI,DPMATF
      IMPLICIT NONE
      LOGICAL(4), PARAMETER :: TDBG=.FALSE.
      ...
      DO IAT=1,NATOMS
        ...
        MAT=(0.D0,0.D0)
        MATP=(0.D0,0.D0)
        DO LN=1,LNX(ISPECIES(IAT))
          L=LOX(LN,ISPECIES(IAT))
          IF(L.NE.LVAL) THEN
            IND=IND+2*L+1
            CYCLE
          ENDIF
...
!     **************************************************************************
      USE DATA_MODULE, ONLY: EIG,OCC,RPOS,RXPOS,TINIT,NATOMS,ATOMNAMES,ISPECIES,INDTYPE,IATMAP,INDMAP,EFERMI,TKINV
      USE RIXS_MODULE, ONLY: EZERO,TEZERO
      USE STRINGS_MODULE
      IMPLICIT NONE
      ...
      INTEGER(4) :: NBB
      INTEGER(4), ALLOCATABLE :: NBARR(:,:)
      CHARACTER(16), ALLOCATABLE :: ATOMSDFT(:)
      INTEGER(4), ALLOCATABLE :: ISPECIESDFT(:)
      INTEGER(4), ALLOCATABLE :: LNX(:)
      INTEGER(4), ALLOCATABLE :: LOX(:,:)
      REAL(8), ALLOCATABLE :: XK(:,:)
      CHARACTER(16) :: ID
...
! TODO: REMOVE SAFELY
      ...
      CALL PDOS$GETR8A('XK',3*NKPT,XK)
      ALLOCATE(TKINV(NKPT))
!     DETERMINE IF K-POINTS HAVE INVERSION SYMMETRY OR FALL ONTO THEMSELVES
      DO IKPT=1,NKPT
...
!     SET REFERENCE ENERGY TO FERMI LEVEL IF NOT SET IN CONTROL FILE
      IF(.NOT.TEZERO) EZERO=EFERMI

!     GET ATOMID, ISPECIES, AND POSITIONS FROM DFT CALCULATION
      ALLOCATE(ATOMSDFT(NAT))
      ALLOCATE(ISPECIESDFT(NAT))
      ALLOCATE(R(3,NAT))
      CALL PDOS$GETI4('NSP',NSP)
      CALL PDOS$GETI4A('ISPECIES',NAT,ISPECIESDFT)
      CALL PDOS$GETCHA('ATOMID',NAT,ATOMSDFT)
      CALL PDOS$GETR8A('R',3*NAT,R)
!     EXTRACT INFORMATION OF DESIRED ATOMS FOR THE RIXS CALCULATION
      ALLOCATE(ISPECIES(NATOMS))
      ALLOCATE(RPOS(3,NATOMS))
      ALLOCATE(RXPOS(3,NATOMS))
      ALLOCATE(IATMAP(NATOMS))
      ALLOCATE(INDMAP(NATOMS))
!     LOOP SELECTED RIXS ATOMS
      DO IAT=1,NATOMS
        TCHK=.FALSE.
!       LOOP ALL DFT ATOMS
        DO IATDFT=1,NAT
          IF(ATOMSDFT(IATDFT).EQ.ATOMNAMES(IAT)) THEN
            TCHK=.TRUE.
            RPOS(:,IAT)=R(:,IATDFT)  ! SAVE MATCHING POSITION
            RXPOS(:,IAT)=MATMUL(INVRBAS,RPOS(:,IAT))  ! CALCULATE REL. POSITION
            ISPECIES(IAT)=ISPECIESDFT(IATDFT)  ! SAVE SPECIES INDEX
            IATMAP(IAT)=IATDFT  ! SAVE INDEX OF ATOM IN DFT CALCULATION
            CYCLE
          ENDIF
        ENDDO
        IF(.NOT.TCHK) THEN
          CALL ERROR$MSG('ATOM NOT FOUND IN DFT CALCULATION')
          CALL ERROR$CHVAL('ATOM',ATOMNAMES(IAT))
          CALL ERROR$STOP('DATA$INIT')
        ENDIF
      ENDDO

! TODO: CHECK IF ORBITAL TYPE SELECTION IS OBSOLETE BECAUSE OF 
!       NCORE, LCORE, NVAL, LVAL SELECTION
...
!     ==  INDMAP POINTS TO (INDEX-1) OF FIRST ATOM PROJECTOR  ==================
      DO IATDFT=1,NAT  ! LOOP THROUGH ALL ATOMS IN DFT CALCULATION
        ISP=ISPECIESDFT(IATDFT)  ! GET SPECIES
        IF(IATDFT.EQ.IATMAP(IAT)) THEN  ! IF ATOM IS SELECTED, SAVE INDEX
          INDMAP(IAT)=IND
          IAT=IAT+1
          IF(IAT.GT.NATOMS) EXIT
        ENDIF
        LMN=0
        DO LN=1,LNX(ISP)
          L=LOX(LN,ISP)
...
! complex conjugate for -kptK
      ...
      SUBROUTINE UNIQUE_STRINGS(N,ARRAY,TCHK,I)  !MARK: UNIQUE_STRINGS
      IMPLICIT NONE
...
!     READ ATOM NAMES
      DO IAT=1,NATOMS
        CALL LINKEDLIST$SELECT(LL_CNTL,'ATOM',IAT)
        ...
        CALL LINKEDLIST$SELECT(LL_CNTL,'..')
      ENDDO
!     CHECK IF EVERY ATOM ONLY APPEARS ONCE
      CALL UNIQUE_STRINGS(NATOMS,ATOMNAMES,TCHK,IAT)
      IF(.NOT.TCHK) THEN
      ...
      CALL TRACE$PASS('ATOMNAMES')
      DO IAT=1,NATOMS
        CALL TRACE$PASS(ATOMNAMES(IAT))
      ENDDO
...
!     DEFAULT WILL BE SET BY CALCULATION OF EFERMI
...
! TODO: IMPLEMENT AQUISITION OF AEPHI WITHOUT USE OF COMMAND LINE EXECUTE
      USE STRINGS_MODULE
      USE DATA_MODULE, ONLY: NATOMS,ISPECIES,CLEN,GIDCORE,CVAL,VLEN,GIDVAL,VVAL,VNPRO !VGRID,CGRID
      IMPLICIT NONE
...
!     ==  GET NUMBER OF PROJECTIONS FROM PDOS MODULE  ==========================
      ...
      PDOSNPRO=LNX(ISPECIES(1))
      DO IAT=1,NATOMS
        IF(LNX(ISPECIES(IAT)).NE.PDOSNPRO) THEN
          CALL ERROR$MSG('NUMBER OF PROJECTIONS FOR SELECTED ATOMS IS NOT EQUAL')
          CALL ERROR$I4VAL('ISPECIES',ISPECIES(IAT))
          CALL ERROR$I4VAL('PDOSNPRO',PDOSNPRO)
          CALL ERROR$I4VAL('LNX',LNX(ISPECIES(IAT)))
          CALL ERROR$MSG('HANDLING OF ATOMS WITH DIFFERENT AUGMENTATION NOT IMPLEMENTED')
...
!     ==  CHECK IF STPFILE EXISTS  =============================================
      ...
      ENDIF
!     ==  EXECUTE STPA.X  ======================================================
      CMD='PAW_STPA.X -S '//TRIM(ADJUSTL(FLAG))// &
     &    ' -O '//TRIM(ADJUSTL(TEMPFILE))//' '//TRIM(ADJUSTL(STPFILE))

      CALL EXECUTE_COMMAND_LINE(-TRIM(ADJUSTL(CMD)),EXITSTAT=EXITSTAT, &
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE DIPOLEMATRIXELEMENTS  !MARK: DIPOLEMATRIXELEMENTS
!     **************************************************************************
!     **  CALCULATES DIPOLE MATRIX ELEMENTS FOR RIXS CALCULATION              **
!     **************************************************************************
      USE DATA_MODULE, ONLY: CLEN,GIDCORE,CVAL,VLEN,GIDVAL,VVAL,VNPRO,DPMATI,DPMATF
      USE RIXS_MODULE, ONLY: SPEC
      IMPLICIT NONE
      COMPLEX(8), PARAMETER :: CI=(0.D0,1.D0)
      REAL(8), PARAMETER :: PI=4.D0*ATAN(1.D0)
! WARNING: CHECK IF HBAR NEEDS TO BE INCLUDED OR IS ONE
      COMPLEX(8), PARAMETER :: FAC=SQRT(4.D0*PI/3.D0)/CI
      INTEGER(4) :: NFIL
      INTEGER(4) :: NSPECTRA
      INTEGER(4) :: NCORE
      INTEGER(4) :: LCORE
      INTEGER(4) :: NVAL
      INTEGER(4) :: LVAL
      INTEGER(4) :: MCORE  ! MAGNETIC QUANTUM NUMBER OF CORE
      INTEGER(4) :: MVAL  ! MAGNETIC QUANTUM NUMBER OF VALENCE
      INTEGER(4) :: IPRO
      INTEGER(4) :: ISPEC
      COMPLEX(8) :: PIXYZ(3)
      COMPLEX(8) :: PFXYZ(3)
      ...
      REAL(8), ALLOCATABLE :: COREARR(:)

      REAL(8), ALLOCATABLE :: TEMPARR(:)
      REAL(8), ALLOCATABLE :: SUMARR(:)
     ...
     &      ALLOCATE(DPMATF(2*LCORE+1,2*LVAL+1,VNPRO,NSPECTRA))
!     ==  INTERPOLATE TO COMMON RADIAL GRID (CHOOSE SMALLER ONE) ===============
!     ==  IF CLEN > VLEN, INTERPOLATE CVAL TO VLEN  ============================
      IF(CLEN.GT.VLEN) THEN
        WRITE(NFIL,FMT='(A,2I5)')'INTERPOLATION RADIAL GRID: CLEN > VLEN',CLEN,VLEN
        GID=GIDVAL
        LEN=VLEN
        ALLOCATE(VALARR(LEN,VNPRO))
        ALLOCATE(COREARR(LEN))
        CALL RADIAL$CHANGEGRID(GIDCORE,CLEN,CVAL,GIDVAL,LEN,COREARR)
        DO I=1,VNPRO
          VALARR(:,I)=VVAL(:,I)
        ENDDO
      ELSE
!     ==  CASE WHERE VLEN > CLEN OR VLEN = CLEN  ===============================
        GID=GIDCORE
        LEN=CLEN
        ALLOCATE(VALARR(LEN,VNPRO))
        ALLOCATE(COREARR(LEN))
!       ==  IF VLEN > CLEN, INTERPOLATE VVAL TO CLEN  ==========================
        IF(VLEN.GT.CLEN) THEN
          WRITE(NFIL,FMT='(A,2I5)')'INTERPOLATION RADIAL GRID: CLEN < VLEN',CLEN,VLEN
          DO I=1,VNPRO
            CALL RADIAL$CHANGEGRID(GIDVAL,VLEN,VVAL(:,I),GIDCORE,LEN,VALARR(:,I))
          ENDDO
        ELSE
          DO I=1,VNPRO
            VALARR(:,I)=VVAL(:,I)
      ...
      ENDIF
      ALLOCATE(RARR(LEN))
      ALLOCATE(TEMPARR(LEN))
      ALLOCATE(SUMARR(LEN))
      CALL RADIAL$R(GID,LEN,RARR)
!     ==  RADIAL INTEGRATION OF DIPOLE MATRIX ELEMENTS  ========================
!     ==  INDEPENDENT OF POLARISATION OF MAGNETIC QUANTUM NUMBER              ==
!     ==  ONLY DEPENDS ON THE RADIAL PARTIAL WAVE OF THE VALENCE STATE        ==
      CALL RADIAL$DERIVE(GID,LEN,COREARR,TEMPARR)
      DO IPRO=1,VNPRO
        CALL RADIAL$INTEGRAL(GID,LEN,VALARR(:,IPRO)*RARR*TEMPARR,RVAR1)
        SUMVAR=RVAR1
        ! WRITE(*,*)'IPRO',IPRO,'SUMVAR1',SUMVAR
        CALL RADIAL$INTEGRAL(GID,LEN,VALARR(:,IPRO)*RARR*COREARR,RVAR1)
        RVAR2=-LCORE
        ! WRITE(*,*)'IPRO',IPRO,'RVAR1',RVAR1,'RVAR2',RVAR2
        IF(LVAL.EQ.LCORE-1) RVAR2=RVAR2+2*LCORE+1
        ! WRITE(*,*)'IPRO',IPRO,'RVAR2',RVAR2
        SUMVAR=SUMVAR+RVAR2*RVAR1
...
!       ==  GET POLARISATION VECTORS  ==========================================
        CALL RIXS$SETPTR(ISPEC)
        PIXYZ=SPEC%PIXYZ
        CALL CNORM2(3,PIXYZ,SUMVAR)
        IF(ABS(SUMVAR-1.D0).GT.1.D-10) THEN
          CALL ERROR$MSG('POLARISATION VECTOR NOT NORMALIZED')
          CALL ERROR$R8VAL('NORM',SUMVAR)
          CALL ERROR$MSG('EXECUTION OF RIXS$POLARISATION MIGHT NOT HAVE HAPPENED IF VALUE IS 0')
          CALL ERROR$STOP('DIPOLEMATRIXELEMENTS')
        ENDIF
        PFXYZ=SPEC%PFXYZ
        CALL CNORM2(3,PFXYZ,SUMVAR)
        IF(ABS(SUMVAR-1.D0).GT.1.D-10) THEN
          CALL ERROR$MSG('POLARISATION VECTOR NOT NORMALIZED')
          CALL ERROR$R8VAL('NORM',SUMVAR)
          CALL ERROR$MSG('EXECUTION OF RIXS$POLARISATION MIGHT NOT HAVE HAPPENED IF VALUE IS 0')
...
!       ==  SUM OVER POLARISATION AND GAUNT COEFFICIENTS  ======================
        DO MCORE=1,2*LCORE+1
          DO MVAL=1,2*LVAL+1
            ...
            ! EXAMPLE P: LVAL=1, MCORE=1,2,3, LLVAL=2,3,4 => PX, PZ, PY
            LLCORE=LCORE*LCORE+MCORE
            LLVAL=LVAL*LVAL+MVAL
...
!     ==  WRITE DIPOLE MATRIX ELEMENTS TO FILE  ================================
      ...
      DEALLOCATE(SUMARR)
                          CALL TRACE$POP
      END SUBROUTINE DIPOLEMATRIXELEMENTS
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE GAUNTPOLSUM(L1,L2,POL,SUM)
!     **************************************************************************
!     **  CALCULATES SUM OVER POLARISATION AND GAUNT COEFFICIENTS             **
!     **  FOR DIPOLE MATRIX ELEMENTS                                          **
!     **************************************************************************
      IMPLICIT NONE
      INTEGER(4), INTENT(IN) :: L1
      INTEGER(4), INTENT(IN) :: L2
      COMPLEX(8), INTENT(IN) :: POL(3)
      COMPLEX(8), INTENT(OUT) :: SUM
      INTEGER(4) :: M1,M2
      REAL(8) :: G
