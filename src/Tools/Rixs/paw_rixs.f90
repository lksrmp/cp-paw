...
! WARNING: CHECK CONJUGATION
      ...
      DO IKPT=1,NKPT
        WGHT=REAL(NKDIV(1)*NKDIV(2)*NKDIV(3)/NSPIN,KIND=8)
        DO ISPIN=1,NSPIN
          ...
          DO WHILE (STATE%NB.GT.ITEN)
            I1=ITEN+1
            I2=MIN(ITEN+10,STATE%NB)
            WRITE(NFILO,FMT=FMTOCC)ITEN,(STATE%OCC(IB)*WGHT,IB=I1,I2)
            ITEN=ITEN+10
          ENDDO
        END DO
      END DO
...
!     ==  AND ORBITALS, REQUIRES INDMAP TO SKIP ATOMS NOT SELECTED  ============
      ...
      ! STATE%VEC(IDIM,IPRO,IB)

      ! IDIM SPINOR COMPONENT

      ! IB=1,STATE%NB SELLECTS BAND

      ! IPRO INDEX OF PROJECTOR, BUILD UP WITH LOOPS
