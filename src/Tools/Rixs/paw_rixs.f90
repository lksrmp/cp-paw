...
! WARNING: INDEXING SHOULD BE CHECKED
        ...
        COMPLEX(8), ALLOCATABLE :: LORENTZ(:) ! (NB2-NOCC) LORENTZIAN DENOMINATOR
...
!       ENERGY GRID PARAMETER
        ...
        REAL(8) :: EBROAD ! GAUSSIAN BROADENING OF ENERGY GRID
...
!     **************************************************************************
      ...
      CALL LINKEDLIST$EXISTD(LL_CNTL,'OMEGA',1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR$MSG('OMEGA NOT FOUND IN !COREHOLE')
        CALL ERROR$STOP('RIXSCNTL$COREHOLE')
      END IF
      CALL LINKEDLIST$GET(LL_CNTL,'OMEGA',1,SETTINGS%OMEGA)
...
!       IEMP: UNOCCUPIED STATE INDEX THAT IS OCCUPIED IN FINAL STATE
      USE RIXS_MODULE, ONLY: SIM,NKPTG,NSPING,KMAP,STATE_TYPE,SETTINGS, &
...
!             RESET AMPLITUDE FOR NEW FINAL STATE
              TAMPL=(0.D0,0.D0)
!             CALCULAR LINEAR DEPENDENCE MATRIX
...
!     COMBINE ALL SPECTRA INFORMATION FROM ALL K POINTS TO SYNC ARRAYS
      DO ISPEC=1,SETTINGS%NSPEC
        ...
        DO IKPT=1,NKPTG
          DO ISPIN=1,NSPING
            CALL GAUSSCONV(SETTINGS%NE,SPEC%E,SPEC%RIXS(:,IKPT,ISPIN),SETTINGS%EBROAD)
...
!     **************************************************************************
      USE RIXS_MODULE, ONLY: OVL,OVLARR,SIM,STATE_TYPE,NKPTG,NSPING,KMAP
      USE MPE_MODULE
      IMPLICIT NONE
      TYPE(STATE_TYPE), POINTER :: S1,S2
      INTEGER(4) :: IKPT,ISPIN
...
&                           OVLARR,TRSTRT,ATOMMAP,KMAP,NKPTG,NSPING
      IMPLICIT NONE
...
!     **************************************************************************
      USE RIXS_MODULE, ONLY: OVL,OVLARR,SIM,KMAP,NKPTG,NSPING
      IMPLICIT NONE
      INTEGER(4) :: IKPT
      INTEGER(4) :: ISPIN
      INTEGER(4) :: NOCC
...
!     **************************************************************************
      USE RIXS_MODULE, ONLY: SIM,KMAP,SETTINGS,NKPTG,NSPING,OVL,OVLARR,STATE_TYPE
      IMPLICIT NONE
      ...
      DO IKPT=1,NKPTG
        ...
        DO ISPIN=1,NSPING
          ...
          S1=>SIM(1)%STATEARR(IKPT,ISPIN)
          S2=>SIM(2)%STATEARR(IKPT,ISPIN)
          ALLOCATE(OVL%LORENTZ(S2%NB-S2%NOCC))
          DO IB=S2%NOCC+1,S2%NB
            CVAR=SIM(2)%ETOT+S2%EIG(IB)-SIM(1)%ETOT
            CVAR=SETTINGS%OMEGA-CVAR+CI*SETTINGS%GAMMA
            OVL%LORENTZ(IB-S2%NOCC)=1/CVAR
          ENDDO ! IB
        ENDDO ! ISPIN
      ENDDO ! IKPT
                          CALL TRACE$POP
...
!     **************************************************************************
      USE RIXS_MODULE, ONLY: OVL,OVLARR,NKPTG,NSPING,SIM,KMAP
      IMPLICIT NONE
      INTEGER(4) :: IKPT
      INTEGER(4) :: ISPIN
      INTEGER(4) :: IEMP
...
!     **************************************************************************
      USE MPE_MODULE
      USE RIXS_MODULE, ONLY: OUTPUT,SIM,NKPTG,NSPING,NSIM,KMAP,RTASK,OVL,OVLARR,STATE_TYPE
      IMPLICIT NONE
      INTEGER(4) :: IKPT
      INTEGER(4) :: ISPIN
      INTEGER(4) :: IS
...
!     **************************************************************************
      ...
      END IF
      IF(FILE.NE.'RIXSOUT'.AND.FILE.NE.'RIXSRAW') THEN
        CALL ERROR$MSG('FILE MUST BE RIXSOUT OR RIXSRAW')
     ...
     &                           TRIM(ADJUSTL(SPECARR(ISPEC)%FILE)))
        ELSE IF(FILE.EQ.'RIXSRAW') THEN
          CALL FILEHANDLER$SETFILE(FILE,.FALSE., &
     &                           TRIM(ADJUSTL(SPECARR(ISPEC)%FILE))//-'RAW')
        END IF
...
!     **************************************************************************
      USE RIXS_MODULE, ONLY: THIS,SIM,OVL,OVLARR,KMAP,NKPTG,NSPING,RTASK,STATE_TYPE
      USE MPE_MODULE
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      TYPE(STATE_TYPE), POINTER :: S
        ...
        DO IKPT=1,THIS%NKPT
          DO ISPIN=1,THIS%NSPIN
            ...
            IF(THISTASK.EQ.RTASK) THEN
              ...
              READ(NFIL)S%NOCC
            END IF
            CALL MPE$SENDRECEIVE('~',RTASK,KMAP(IKPT),S%NB)
            CALL MPE$SENDRECEIVE('~',RTASK,KMAP(IKPT),S%NOCC)
            ALLOCATE(S%EIG(S%NB))
            IF(THISTASK.EQ.RTASK) THEN
              READ(NFIL)S%EIG
            END IF
            CALL MPE$SENDRECEIVE('~',RTASK,KMAP(IKPT),S%EIG)
          ENDDO
        ENDDO
        CALL RIXS$UNSELECT
      ENDDO
