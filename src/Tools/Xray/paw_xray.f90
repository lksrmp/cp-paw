...
!     **************************************************************************
      ...
      INTEGER(4) :: NKPTG  ! #(KPOINTS TOTAL)
      INTEGER(4) :: NSPING ! #(SPINS TOTAL)
      ! TODO: CHECK IF ADETSUM IS NECESSARY
      REAL(8) :: ADETSUM ! SUM_K SUM_S |ADET|^2
...
!     **************************************************************************
      ...
      REAL(8), ALLOCATABLE :: ADETSUM(:,:) ! (NKPT,NSPIN)
...
!     **                                                                      **
!     **************************************************************************
      USE XCNTL_MODULE, ONLY: LL_CNTL,ADET,BASISWGHT
      USE LINKEDLIST_MODULE
      IMPLICIT NONE
...
!     **************************************************************************
      USE XCNTL_MODULE, ONLY: TOVERLAP,ADET,BASISWGHT
      IMPLICIT NONE
...
!     **************************************************************************
      ...
      USE SHARED_DATA_MODULE, ONLY: ABSORB,ADETSUM
      USE MPE_MODULE
      IMPLICIT NONE
      ...
      DO IKPT=1,NKPT
        ...
        DO ISPIN=1,NSPIN
          ...
          ! 0TH ORDER
          ADETSUM(IKPT,ISPIN)=ADETSUM(IKPT,ISPIN)+ABS(ADET)**2

          DO IEMP=1,NB2-NOCC
            ! 1ST ORDER
            DO I=1,NOCC
              ADETSUM(IKPT,ISPIN)=ADETSUM(IKPT,ISPIN)+ABS(KMAT(IEMP,I))**2*ABS(ADET)**2
            ENDDO
      ...
      CALL STATE$UNSELECT
      DEALLOCATE(WKPT)
      ! TODO: CHECK IF ADETSUM IS USED CORRECTLY FOR NPSIN=1
      CALL MPE$COMBINE('~','+',ADETSUM)
      IF(NSPIN.EQ.2) THEN
        ADETSUM(:,1)=ADETSUM(:,1)*ADETSUM(:,2)
      ELSE
        ! IF NSPIN=1, ASSUME ADETSUM IS THE SAME FOR BOTH SPINS
        ADETSUM(:,1)=ADETSUM(:,1)*ADETSUM(:,1)
      END IF
      ...
      DO ISPEC=1,NSPEC
        ...
        IF(TBASISWGHT) THEN
          ! MULTIPLY EACH SPECTRUM BY K POINT BASIS WEIGHT
          DO IKPT=1,NKPT
            DO ISPIN=1,NSPIN
              THIS%SPECTRUM(IKPT,ISPIN,:)=THIS%SPECTRUM(IKPT,ISPIN,:)/ADETSUM(IKPT,1)
            ENDDO
          ENDDO
        END IF
        CALL XAS$UNSELECT
...
!     **************************************************************************
      ...
      USE SHARED_DATA_MODULE, ONLY: ADETSUM
      USE STRINGS_MODULE
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      INTEGER(4) :: NTASKS,THISTASK,RTASK
...
!     **************************************************************************
      ...
      IF(TXAS) THEN
        ...
        IF(ALLOCATED(ABSORB).OR.ALLOCATED(ADETSUM)) THEN
          CALL TRACE$PASS('ABSORB OR ADETSUM ALREADY CALCULATED IN XAS')
          IF(SP) THEN
          ...
          END IF
        ELSE
          CALL ERROR$MSG('ABSORB OR ADETSUM NOT CALCULATED IN XAS DESPITE XAS ACTIVE')
          CALL ERROR$STOP('RIXS_ABSORB')
        END IF
      END IF
      ...
      IF(.NOT.TXAS) THEN
        IF(ALLOCATED(ABSORB)) THEN
          ...
          CALL ERROR$STOP('RIXS_ABSORB')
        END IF
        IF(ALLOCATED(ADETSUM)) THEN
          CALL ERROR$MSG('SAFEGUARD FUNCTION:')
          CALL ERROR$MSG('ADETSUM ALLOCATED BUT XAS NOT ACTIVE')
          CALL ERROR$STOP('RIXS_ABSORB')
        END IF
      END IF
        ...
        DO ISPIN=1,NSPIN
          ...
          DO IOCC=1,NOCC
            ...
            DO IEMP=1,NB1-NOCC
              ...
              IF(TBASISWGHT) THEN
                AMPLTOT(:)=AMPLTOT(:)/ADETSUM(IKPT,1)
              END IF
              ! TODO: CHECK IF WE NEED WKPT HERE IN ANY FORM
...
!     **************************************************************************
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ADETSUM = ADETSUM + ABS(ADET(IKPT,ISPIN))**2
        ENDDO
      ENDDO
...
!     **************************************************************************
      ...
      END IF
      WRITE(NFIL,'(A12,A12,A12)') 'IKPT','ADETSUM','1/ADETSUM'
      DO IKPT=1,NKPT
        WRITE(NFIL,'(I12,F12.8)') IKPT,ADETSUM(IKPT,1),1.D0/ADETSUM(IKPT,1)
      ENDDO ! END IKPT
                          CALL TRACE$POP
      RETURN
