...
!     **************************************************************************
      ...
      COMPLEX(8), ALLOCATABLE :: DIPOLE(:,:) ! (3,NB2) DIPOLE MATRIX ELEMENTS FOR SIMULATION 2
      COMPLEX(8), ALLOCATABLE :: DIPOLEGS(:,:) ! (3,NB1) DIPOLE MATRIX ELEMENTS FOR SIMULATION 1

      ! MAXIMUM OVERLAP INFO
      INTEGER(4), ALLOCATABLE :: IND(:)   ! (NOCC) ASCENDING INDICES OF EXCITE OCC. STATE TO MAX OVERLAP
      INTEGER(4), ALLOCATABLE :: INDE(:)  ! (NB2-NOCC) ASCENDING INDICES OF EXCITE UNOCC. STATE TO MAX OVERLAP
      INTEGER(4) :: NSWAPPED=0 ! NUMBER OF SWAPPED STATES IN MAXIMUM OVERLAP
      INTEGER(4), ALLOCATABLE :: MISSING(:) ! (NSWAPPED) ASCENDING INDICES OF EXCITED STATE OCC. STATES NOT IN MAX OVERLAP
      INTEGER(4), ALLOCATABLE :: EXTRA(:) ! (NSWAPPED) ASCENDING INDICES OF EXCITED STATE UNOCC. STATES IN MAX OVERLAP
      REAL(8) :: ESWAP=0.D0 ! ENERGY DIFF BETWEEN SWAPPED STATES FROM KS EIGENVALUES
      ...
      REAL(8) :: R2CORE(2) ! (NSIM) <C|R^2|C> FOR CORE ORBITAL IN SIMULATION 1 AND 2
...
!     **************************************************************************
      ...
      ! SPHERICAL COORDINATES FOR KDIR WITH SURFACE NORMAL AS Z-AXIS
      ! ONLY AVAILABLE IF GIVEN BY USER
      REAL(8) :: THETAPHI(2) ! SPHERICAL ANGLES THETA,PHI FOR KDIR IN RADIANS
      LOGICAL(4) :: TTHETAPHI=.FALSE. ! CHECK IF THETAPHI IS SET
...
!     ==========================================================================
      ...
      ! CALCULATE OVERLAP BEFORE REPORTS: XAS$REPORT MAY TRIGGER THE
      ! AUTOMATIC ENERGY WINDOW, WHICH REQUIRES THE MAXIMUM-OVERLAP
      ! MAPPINGS IND AND INDE TO BE AVAILABLE ON ALL TASKS
      CALL XRAY$OVERLAP
      ...
      ! CALCULATE INCIDENT VECTOR FROM ANGLES IF NECESSARY
      CALL XAS$KDIRFROMTHETAPHI
...
!     **                                                                      **
!     **************************************************************************
      USE XCNTL_MODULE, ONLY: LL_CNTL,ADET,BASISWGHT,NORMALIZE,MAXOVERLAP, &
     &                        MOMENTUMTRANSFER
      USE LINKEDLIST_MODULE
      IMPLICIT NONE
...
!     **************************************************************************
      ...
      END IF
      ! CHECK WETHER TO ORDER EXCITED STATES TO MAXIMIZE OVERLAP WITH GROUND
      CALL LINKEDLIST$EXISTD(LL_CNTL,'MAXOVERLAP',1,TCHK)
      IF(TCHK) THEN
        CALL LINKEDLIST$GET(LL_CNTL,'MAXOVERLAP',1,MAXOVERLAP)
      ELSE
        MAXOVERLAP=.TRUE. ! DEFAULT IS TO ORDER EXCITED STATES TO MAXIMIZE OVERLAP WITH GROUND
      END IF
...
!     **************************************************************************
      ...
      ! KDIR VECTOR OR THETAPHI ANGLES
      CALL LINKEDLIST$EXISTD(LL_CNTL,'KDIR',1,TKDIRDEF)
      CALL LINKEDLIST$EXISTD(LL_CNTL,'THETAPHI[DEG]',1,TTHETAPHIDEF)

      ! CATCH IF BOTH ARE GIVEN AS DEFAULTS
      IF(TTHETAPHIDEF.AND.TKDIRDEF) THEN
        CALL ERROR$MSG('BOTH THETAPHI AND KDIR ARE DEFINED')
        CALL ERROR$MSG('ONLY ONE OF THESE OPTIONS CAN BE USED TO DEFINE KDIR')
        CALL ERROR$STOP('XCNTL$XAS')
      END IF
      ...
      IF(TKDIRDEF) THEN
        ...
        IF(NORM2(KDIR).LT.1.D-6) THEN
          ...
          CALL ERROR$STOP('XCNTL$XAS')
        END IF
      ELSE IF(TTHETAPHIDEF) THEN
        CALL LINKEDLIST$GET(LL_CNTL,'THETAPHI[DEG]',1,THETAPHI)
      END IF
      ...
      IF(TPOLDEF) THEN
        ...
        IF(SQRT(SUM(ABS(POL)**2)).LT.1.D-6) THEN
      ...
      DO ISPEC=1,NSPEC
        ...
        ! KDIR VECTOR AND THETAPHI ANGLES
        CALL LINKEDLIST$EXISTD(LL_CNTL,'KDIR',1,TKDIR)
        CALL LINKEDLIST$EXISTD(LL_CNTL,'THETAPHI[DEG]',1,TTHETAPHI)

        ! CATCH IF BOTH ARE GIVEN IN SAME SPECTRUM
        IF(TKDIR.AND.TTHETAPHI) THEN
          CALL ERROR$MSG('BOTH THETAPHI AND KDIR ARE DEFINED')
          CALL ERROR$MSG('ONLY ONE OF THESE OPTIONS CAN BE USED TO DEFINE KDIR')
          CALL ERROR$I4VAL('ISPEC',ISPEC)
          CALL ERROR$STOP('XCNTL$XAS')
        END IF
        ! CATCH IF NONE ARE GIVEN EITHER IN SPECTRUM OR AS DEFAULTS
        IF(.NOT.TKDIR.AND..NOT.TTHETAPHI.AND..NOT.TKDIRDEF.AND..NOT.TTHETAPHIDEF) THEN
          CALL ERROR$MSG('KDIR OR THETAPHI VECTOR MUST BE SET')
          CALL ERROR$I4VAL('ISPEC',ISPEC)
          CALL ERROR$STOP('XCNTL$XAS')
        END IF
        ...
        ELSE IF(TTHETAPHI) THEN
          CALL LINKEDLIST$GET(LL_CNTL,'THETAPHI[DEG]',1,RVAR2)
          CALL XAS$SETR8A('THETAPHI',2,RVAR2*DEGREE)
        ELSE IF(TTHETAPHIDEF) THEN
          CALL XAS$SETR8A('THETAPHI',2,THETAPHI*DEGREE)
        ELSE IF(TKDIRDEF) THEN
          CALL XAS$SETR8A('KDIR',3,KDIR)
        ELSE
          CALL ERROR$MSG('SAFETY CHECK: KDIR OR THETAPHI VECTOR MUST BE SET')
        ...
        IF(TPOL) THEN
          ...
          IF(SQRT(SUM(ABS(CVAR2)**2)).LT.1.D-6) THEN
            CALL ERROR$MSG('POLARIZATION VECTOR CANNOT BE ZERO')
            CALL ERROR$I4VAL('ISPEC',ISPEC)
            CALL ERROR$STOP('XCNTL$XAS')
          END IF
...
!     **************************************************************************
      USE XCNTL_MODULE, ONLY: TOVERLAP,ADET,BASISWGHT,NORMALIZE,MAXOVERLAP, &
     &                        MOMENTUMTRANSFER
      IMPLICIT NONE
...
!     ** REQUIRES BOTH SIMULATIONS INITIALIZED AND NONE SELECTED              **
!     **************************************************************************
      USE SIMULATION_MODULE, ONLY: GROUND,EXCITE,SELECTED,THIS
      IMPLICIT NONE
      REAL(8), PARAMETER :: TOL=1.D-8
!     **************************************************************************
                          CALL TRACE$PUSH('SIMULATION$CONSISTENCY')
      IF(SELECTED) THEN
        CALL ERROR$MSG('SAFEGUARD FUNCTION:')
        CALL ERROR$MSG('CANNOT CHECK CONSISTENCY WHEN SIMULATION IS SELECTED')
        CALL ERROR$CHVAL('SELECTED SIMULATION ID: ',THIS%ID)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(.NOT.GROUND%INITIALIZED.OR..NOT.EXCITE%INITIALIZED) THEN
        ...
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(GROUND%NAT.NE.EXCITE%NAT) THEN
        CALL ERROR$MSG('NAT DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$I4VAL('GROUND NAT: ',GROUND%NAT)
        CALL ERROR$I4VAL('EXCITE NAT: ',EXCITE%NAT)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(GROUND%NSPIN.NE.EXCITE%NSPIN) THEN
        CALL ERROR$MSG('NSPIN DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$I4VAL('GROUND NSPIN: ',GROUND%NSPIN)
        CALL ERROR$I4VAL('EXCITE NSPIN: ',EXCITE%NSPIN)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(GROUND%NKPT.NE.EXCITE%NKPT) THEN
        CALL ERROR$MSG('NKPT DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$I4VAL('GROUND NKPT: ',GROUND%NKPT)
        CALL ERROR$I4VAL('EXCITE NKPT: ',EXCITE%NKPT)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(ANY(GROUND%NKDIV.NE.EXCITE%NKDIV)) THEN
        CALL ERROR$MSG('NKDIV DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$I4VAL('GROUND NKDIV1: ',GROUND%NKDIV(1))
        CALL ERROR$I4VAL('GROUND NKDIV2: ',GROUND%NKDIV(2))
        CALL ERROR$I4VAL('GROUND NKDIV3: ',GROUND%NKDIV(3))
        CALL ERROR$I4VAL('EXCITE NKDIV1: ',EXCITE%NKDIV(1))
        CALL ERROR$I4VAL('EXCITE NKDIV2: ',EXCITE%NKDIV(2))
        CALL ERROR$I4VAL('EXCITE NKDIV3: ',EXCITE%NKDIV(3))
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(GROUND%TSHIFT.NEQV.EXCITE%TSHIFT) THEN
        CALL ERROR$MSG('TSHIFT DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$L4VAL('GROUND TSHIFT: ',GROUND%TSHIFT)
        CALL ERROR$L4VAL('EXCITE TSHIFT: ',EXCITE%TSHIFT)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(MAXVAL(ABS(GROUND%RBAS-EXCITE%RBAS)).GT.TOL) THEN
        CALL ERROR$MSG('RBAS DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$R8VAL('MAX DIFF: ',MAXVAL(ABS(GROUND%RBAS-EXCITE%RBAS)))
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
      IF(ABS(GROUND%NEL-EXCITE%NEL).GT.TOL) THEN
        CALL ERROR$MSG('NEL DIFFERS BETWEEN SIMULATIONS')
        CALL ERROR$R8VAL('GROUND NEL: ',GROUND%NEL)
        CALL ERROR$R8VAL('EXCITE NEL: ',EXCITE%NEL)
        CALL ERROR$STOP('SIMULATION$CONSISTENCY')
      END IF
                          CALL TRACE$POP
      RETURN
...
!     ** REQUIRES SELECTED SIMULATION                                         **
!     **************************************************************************
      USE SIMULATION_MODULE, ONLY: SELECTED,THIS,ATOMMAP
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      INTEGER(4), INTENT(IN) :: LEN
...
!     **************************************************************************
      ...
      USE PERIODICTABLE_MODULE
      IMPLICIT NONE
      REAL(8), PARAMETER :: RAUG=1.D0 ! RASA*RAUG IS AUGMENTATION RADIUS FOR DOS
...
! TODO: THIS SHOULD BE FIXED IN A FUTURE ITERATION
            ...
            CALL RADIAL$VALUE(GID,NR,TEMP1,RAUG*THIS(ISP)%RASA,OV(LN1,LN2,ISP))
...
!     **************************************************************************
! TODO: CHECK CALCULATION OF INFTOTKPT ETC.
     ...
     &                      TOTAL,GOOG,NORM,INFTOTAL
      ...
      REAL(8) :: ESWAP
      INTEGER(4), ALLOCATABLE :: INDE(:) ! (NB2-NOCC) INDICES OF UNOCCUPIED STATES FOLLOWING SWAPS
      COMPLEX(8) :: CVAR
      COMPLEX(8) :: PTERM ! <C|O^+ P_K O|C>
      COMPLEX(8) :: PDTERM ! <C|O^+ P_K^+ O|C>
      COMPLEX(8) :: PDPTERM ! <C|O^+ P_K^+ P_K O|C>
      REAL(8) :: SIGMA
      ...
      COMPLEX(8), ALLOCATABLE :: INFTOTKPT(:) ! (NKPT) DEBUG DIAGNOSTIC
      COMPLEX(8), ALLOCATABLE :: ADETKPT(:) ! (NKPT) DEBUG DIAGNOSTIC
      REAL(8), ALLOCATABLE :: GOOGKPT(:) ! <G|O^+ O|G> FOR EACH KPOINT
      LOGICAL(4), PARAMETER :: DEBUG=.TRUE. ! PER-KPOINT DIAGNOSTICS
!     **************************************************************************
      ...
      IF(DEBUG) THEN
        ALLOCATE(GOOGKPT(NKPT))
        ALLOCATE(INFTOTKPT(NKPT))
        ALLOCATE(ADETKPT(NKPT))
      END IF
      ...
      DO IKPT=1,NKPT
        ...
        DO ISPIN=1,NSPIN
          ...
          ! SET TWICE FOR EACH KPOINT, ONCE FOR EACH SPIN, WITH THE SAME VALUE
          IF(DEBUG) ADETKPT(IKPT)=ADET*ADETOP
          CALL OVERLAP$GETI4('NB1',NB1)
          CALL OVERLAP$GETI4('NB2',NB2)
          CALL OVERLAP$GETI4('NOCC',NOCC)
          ALLOCATE(EIG(NB2))
          CALL STATE$GETR8A('EIG',IKPT,ISPIN,NB2,EIG)
          CALL OVERLAP$GETR8('ESWAP',ESWAP)
          ALLOCATE(INDE(NB2-NOCC))
          CALL OVERLAP$GETI4A('INDE',NB2-NOCC,INDE)
          ALLOCATE(DIPOLE(3,NB2))
          CALL OVERLAP$GETC8A('DIPOLE',3*NB2,DIPOLE)
          ALLOCATE(DIPOLEGS(3,NB1))
          CALL OVERLAP$GETC8A('DIPOLEGS',3*NB1,DIPOLEGS)
          ALLOCATE(KMAT(NB2-NOCC,NOCC))
          CALL OVERLAP$K(NB2-NOCC,NOCC,KMAT)
          ALLOCATE(ABSORB(IKPT,ISPIN)%AMPL(3,NB2-NOCC))

          ! SUM_N |<PSI_N|EPSILON*R|C>|^2 FOR N OCCUPIED STATES IN GROUND
          ! INTEGRATED OVER POLARIZATION (FACTOR 1/3 APPLIED LATER)
          DO I=1,NOCC
            DO J=1,3
              GOOG=GOOG+TOTALWKPT*ABS(DIPOLEGS(J,I))**2
              IF(TNEGATIVEKPT) THEN
                GOOG=GOOG+TOTALWKPT*ABS(CONJG(DIPOLEGS(J,I)))**2
              END IF
            ENDDO
          ENDDO
          IF(DEBUG) THEN
            DO I=1,NOCC
              DO J=1,3
                GOOGKPT(IKPT)=GOOGKPT(IKPT)+WKPT(IKPT)*ABS(DIPOLEGS(J,I))**2
              END DO
            END DO
          END IF
          ...
          DO I=1,NOCC
            DO J=1,NOCC
              DO M=1,3
                PTERM=PTERM+CONJG(DIPOLEGS(M,I))*CONJG(AINV(I,J))*DIPOLE(M,J)
                PDTERM=PDTERM+CONJG(DIPOLE(M,I))*AINV(J,I)*DIPOLEGS(M,J)
              ENDDO
              DO K=1,NOCC
                DO M=1,3
                  PDPTERM=PDPTERM+CONJG(DIPOLE(M,I))*AINV(J,I)*CONJG(AINV(J,K))*DIPOLE(M,K)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
          ! <C|O^+ (1 - P_K - P_K^+ + P_K^+ P_K) O|C>
          INFTOTALKPT=TOTALWKPT*(PTERM+PDTERM-PDPTERM)
          IF(TNEGATIVEKPT) THEN
            ...
            INFTOTALKPT=INFTOTALKPT+TOTALWKPT*CONJG(PTERM+PDTERM-PDPTERM)
          END IF
          IF(TADET) THEN
          ...
          IF(DEBUG) THEN
            INFTOTKPT(IKPT)=INFTOTKPT(IKPT)+TOTALWKPT*(PTERM+PDTERM-PDPTERM)
            IF(TNEGATIVEKPT) THEN
              INFTOTKPT(IKPT)=INFTOTKPT(IKPT)+TOTALWKPT*CONJG(PTERM+PDTERM-PDPTERM)
            END IF
          END IF


          ! SUM_K WKPT(K) * |ADET_K|^2 = SUM_K WKPT(K) * |ADET_K^UP * ADET_K^DOWN|^2
          ! DIVIDE BY NSPIN TO AVOID DOUBLE COUNTING IF NSPIN=2
          ! WKPT TAKES CARE OF NEGATIVE K-POINTS
          ADETSUM=ADETSUM+WKPT(IKPT)*ABS(ADET*ADETOP)**2/REAL(NSPIN,KIND=8)
          DEALLOCATE(AINV)
          ...
          DO IEMP=1,NB2-NOCC
            ...
            EFINAL=EEXCITE+EIG(INDE(IEMP))+ESWAP
            ...
            DO ISPEC=1,NSPEC
              ...
              ! ADD TOTAL SPECTRUM
              TOTAL=TOTAL+TOTALWKPT*ABS(AMPL(1))**2+TOTALWKPT*ABS(AMPL(2))**2+TOTALWKPT*ABS(AMPL(3))**2
              IF(TNEGATIVEKPT) THEN
...
!       INFTOTAL IS DIVIDED BY 3 ABOVE
      ...
      GOOG=R2CORE(1)-GOOG
      ! COMBINE ADETSUM
      CALL MPE$COMBINE('~','+',ADETSUM)
      ! COMBINE INFTOTAL
      CALL MPE$COMBINE('~','+',INFTOTAL)
      INFTOTAL=INFTOTAL/3.D0 ! DIVIDE BY 3 FOR POLARIZATION AVERAGE
      INFTOTAL=R2CORE(1)*ADETSUM-INFTOTAL
      NORM=GOOG/INFTOTAL

      IF(DEBUG) THEN
        CALL MPE$COMBINE('~','+',ADETKPT)
        ! ADETKPT GETS SUMMED OVER SPINS, SO DIVIDE BY NSPIN TO AVOID DOUBLE COUNTING
        ADETKPT(:)=ADETKPT(:)/REAL(NSPIN,KIND=8)
        CALL MPE$COMBINE('~','+',INFTOTKPT)
        DO IKPT=1,NKPT
          INFTOTKPT(IKPT)=ABS(ADETKPT(IKPT))**2*(R2CORE(1)*WKPT(IKPT)-INFTOTKPT(IKPT)/3.D0)
        END DO
        CALL MPE$COMBINE('~','+',GOOGKPT)
        DO IKPT=1,NKPT
          GOOGKPT(IKPT)=WKPT(IKPT)*R2CORE(1)-GOOGKPT(IKPT)/3.D0
        ENDDO
        IF(THISTASK.EQ.RTASK) THEN
          PRINT *, 'GOOGKPT: ', GOOGKPT
          PRINT *, 'GOOG: ', GOOG
          PRINT *, 'SUM OF GOOGKPT/GOOG: ', SUM(GOOGKPT)/GOOG
          PRINT *, 'INFTOTKPT: ', REAL(INFTOTKPT,KIND=8)
          PRINT *, 'INFTOTAL: ', INFTOTAL
          PRINT *, 'SUM OF INFTOTKPT/INFTOTAL: ', SUM(REAL(INFTOTKPT,KIND=8))/INFTOTAL
        END IF
        DEALLOCATE(GOOGKPT)
        DEALLOCATE(INFTOTKPT)
        DEALLOCATE(ADETKPT)
      END IF
      DEALLOCATE(WKPT)
        ...
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            ...
            IF(TNORMALIZE) THEN
              ...
              THIS%SPECTRUM(IKPT,ISPIN,:)=THIS%SPECTRUM(IKPT,ISPIN,:)*NORM
            END IF  
            ...
            IF(TNORMALIZE) THEN
              ...
              THIS%SRAW(IKPT,ISPIN)%V=THIS%SRAW(IKPT,ISPIN)%V*NORM
            END IF
          ENDDO
        ENDDO
        CALL XAS$UNSELECT
...
!     **************************************************************************
     ...
     &                      TOTAL,GOOG,NORM
      ...
      USE STRINGS_MODULE
      IMPLICIT NONE
      ! RECL IS MANUALLY SET IN FILEHANDLER_OPEN AS 1000
...
!     **************************************************************************
      ...
      CALL XCNTL$GETL4('BASISWGHT',TBASISWGHT)
      CALL XCNTL$GETL4('NORMALIZE',TNORMALIZE)
      CALL XCNTL$GETL4('MAXOVERLAP',TMAXOVERLAP)
      WRITE(NFIL,'(A)')''
      WRITE(NFIL,'(80("#"))')
      ...
      END IF
      IF(TMAXOVERLAP) THEN
        WRITE(NFIL,FMT='(A)')'MAXIMUM OVERLAP USED (MAXOVERLAP=T)'
      ELSE
        WRITE(NFIL,FMT='(A)')'NO MAXIMUM OVERLAP USED (MAXOVERLAP=F)'
      END IF
      IF(TBASISWGHT) THEN
      ...
      DO ISPEC=1,NSPEC
        ...
        IF(THIS%TTHETAPHI) THEN
          WRITE(NFIL,FMT='(A12,F10.4,A12,F10.4)') &
     &     'THETA[°]:',THIS%THETAPHI(1)/DEGREE,'PHI[°]:',THIS%THETAPHI(2)/DEGREE
        END IF
      ...
      IF(THISTASK.EQ.RTASK) THEN
        ...
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            ...
            ALLOCATE(INDE(NB-NOCC))
            CALL OVERLAP$SELECT(IKPT,ISPIN)
            CALL OVERLAP$GETI4A('INDE',NB-NOCC,INDE)
            CALL OVERLAP$GETR8('ESWAP',ESWAP)
            CALL OVERLAP$UNSELECT
            DO IFINAL=1,NB-NOCC
              EFINAL=EEXCITE+EIG(INDE(IFINAL))+ESWAP
              E=EFINAL-EGROUND
...
!     **************************************************************************
     ...
     &                      EFAC,TOTAL,GOOG,NORM,INFTOTAL
      ...
      LOGICAL(4) :: TNORMALIZE
      LOGICAL(4) :: TMAXOVERLAP
      INTEGER(4) :: IKPT,ISPIN,NKPT,NSPIN
      INTEGER(4) :: NSWAPPED
      INTEGER(4),ALLOCATABLE :: SWAPPED(:)
      INTEGER(4) :: I
!     **************************************************************************
     ...
     &     'THETA[°]:',THIS%THETAPHI(1)/DEGREE,'PHI[°]:',THIS%THETAPHI(2)/DEGREE
      ...
      END IF
      IF(TMAXOVERLAP) THEN
        CALL OVERLAP$SELECT(1,1)
        CALL OVERLAP$GETI4('NKPT',NKPT)
        CALL OVERLAP$GETI4('NSPIN',NSPIN)
        CALL OVERLAP$UNSELECT
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            CALL OVERLAP$SELECT(IKPT,ISPIN)
            CALL OVERLAP$GETI4('NSWAPPED',NSWAPPED)
            IF(NSWAPPED.GT.0) THEN
              ALLOCATE(SWAPPED(NSWAPPED))
              CALL OVERLAP$GETI4A('MISSING',NSWAPPED,SWAPPED)
              WRITE(NFIL,FMT='("# ",A,I4,A,I4)')'KPT:',IKPT,' SPIN:',ISPIN
              WRITE(NFIL,FMT='("# ")',ADVANCE='NO')
              DO I=1,NSWAPPED
                WRITE(NFIL,FMT='(I4,1X)',ADVANCE='NO')SWAPPED(I)
              ENDDO
              WRITE(NFIL,FMT='("->")',ADVANCE='NO')
              CALL OVERLAP$GETI4A('EXTRA',NSWAPPED,SWAPPED)
              DO I=1,NSWAPPED
                WRITE(NFIL,FMT='(I4,1X)',ADVANCE='NO')SWAPPED(I)
              ENDDO
              WRITE(NFIL,FMT='(" ")')
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XAS$KDIRFROMTHETAPHI  ! MARK: XAS_KDIRFROMTHETAPHI
!     **************************************************************************
!     ** SET KDIR FROM THETA AND PHI ANGLES                                   **
!     **************************************************************************
      USE XAS_MODULE, ONLY: TACTIVE,TINITIALIZE,SELECTED,THIS,NSPEC
      IMPLICIT NONE
      INTEGER(4) :: ISPEC
      REAL(8) :: KDIR(3)
      REAL(8) :: ROT(3,3)
...
!     **************************************************************************
      ...
      IF(.NOT.TINITIALIZE) THEN
        CALL ERROR$MSG('XAS NOT INITIALIZED')
        CALL ERROR$STOP('XAS_KDIRFROMTHETAPHI')
      END IF
      IF(SELECTED) THEN
        CALL ERROR$MSG('SAFEGUARD FUNCTION:')
        ...
        CALL ERROR$STOP('XAS_KDIRFROMTHETAPHI')
      END IF
      ZNORMAL=(/0.D0,0.D0,1.D0/)
      DO ISPEC=1,NSPEC
        CALL XAS$ISELECT(ISPEC)
        IF(THIS%TTHETAPHI) THEN
          KDIR(1)=SIN(THIS%THETAPHI(1))*COS(THIS%THETAPHI(2))
          KDIR(2)=SIN(THIS%THETAPHI(1))*SIN(THIS%THETAPHI(2))
          KDIR(3)=COS(THIS%THETAPHI(1))
          ! MAKE VECTOR POINTING INTO THE SURFACE
          KDIR=-KDIR
          CALL ROTMATFROMN(ZNORMAL,THIS%NORMAL,ROT)
          THIS%KDIR=MATMUL(ROT,KDIR)
        END IF
        CALL XAS$UNSELECT()
      ENDDO
      END SUBROUTINE XAS$KDIRFROMTHETAPHI
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
...
!     **************************************************************************
      ...
      ELSE IF(ID.EQ.'KDIR') THEN
        ...
        END IF
        THIS%KDIR(:)=VAL(:)
      ELSE IF(ID.EQ.'THETAPHI') THEN
        IF(LEN.NE.2) THEN
          CALL ERROR$MSG('XAS SETR8A THETAPHI LEN NOT 2')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$STOP('XAS$SETR8A')
        END IF
        THIS%THETAPHI(:)=VAL(:)
...
!     **************************************************************************
      ...
      WRITE(NFIL,'(A4,X,A5,X,A21,X,A10,X,A16)') 'IKPT','ISPIN','ADET','|ADET|^2','SWAPPED ORBITALS'
      DO IKPT=1,NKPTG
        DO ISPIN=1,NSPING
          CALL OVERLAP$SELECT(IKPT,ISPIN)
          WRITE(NFIL,FMT=-'(I4,X,I5,X,(F10.7,SP,F10.7,"I ",S),X,F10.7,X)',ADVANCE='NO') &
     &                                       IKPT,ISPIN,THIS%ADET,ABS(THIS%ADET)**2
          IF(THIS%NSWAPPED.GT.0) THEN
            DO I=1,THIS%NSWAPPED
              WRITE(NFIL,FMT='(I4,X)',ADVANCE='NO') THIS%MISSING(I)
            ENDDO
            WRITE(NFIL,'(A)',ADVANCE='NO') '->'
            DO I=1,THIS%NSWAPPED
              WRITE(NFIL,FMT='(I4,X)',ADVANCE='NO') THIS%EXTRA(I)
            ENDDO
            WRITE(NFIL,FMT='(A10,F10.7)') 'ESWAP:',THIS%ESWAP
          ELSE
            WRITE(NFIL,'(A)') ''
          END IF
          CALL OVERLAP$UNSELECT
        ENDDO
      ENDDO
      WRITE(NFIL,'(A12,2F14.7)') 'R2CORE:',R2CORE(:)
                          CALL TRACE$POP
      RETURN
...
!        JUST USES VALUE DETERMINED FOR GROUND
      ...
      ELSE IF(ID.EQ.'NSWAPPED') THEN
        IF(VAL.LT.0) THEN
          CALL ERROR$MSG('NSWAPPED CANNOT BE NEGATIVE')
          CALL ERROR$I4VAL('NSWAPPED: ',VAL)
          CALL ERROR$STOP('OVERLAP$SETI4')
        END IF
        THIS%NSWAPPED=VAL
      ELSE
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE OVERLAP$GETI4A(ID,LEN,VAL)  ! MARK: OVERLAP$GETI4A
!     **************************************************************************
!     ** GET INTEGER VALUE ARRAY FROM SELECTED OVERLAP                       **
!     ** REQUIRES SELECTED OVERLAP                                           **
!     **************************************************************************
      USE OVERLAP_MODULE, ONLY: THIS,SELECTED
      IMPLICIT NONE
...
!     **************************************************************************
      IF(.NOT.SELECTED) THEN
        CALL ERROR$MSG('NO OVERLAP SELECTED')
        CALL ERROR$STOP('OVERLAP$GETI4A')
      END IF
      IF(ID.EQ.'MISSING') THEN
        IF(LEN.NE.THIS%NSWAPPED) THEN
          CALL ERROR$MSG('LEN MUST BE NSWAPPED FOR MISSING')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NSWAPPED: ',THIS%NSWAPPED)
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        IF(.NOT.ALLOCATED(THIS%MISSING)) THEN
          CALL ERROR$MSG('MISSING ARRAY NOT ALLOCATED')
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        VAL=THIS%MISSING
      ELSE IF(ID.EQ.'EXTRA') THEN
        IF(LEN.NE.THIS%NSWAPPED) THEN
          CALL ERROR$MSG('LEN MUST BE NSWAPPED FOR EXTRA')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NSWAPPED: ',THIS%NSWAPPED)
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        IF(.NOT.ALLOCATED(THIS%EXTRA)) THEN
          CALL ERROR$MSG('EXTRA ARRAY NOT ALLOCATED')
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        VAL=THIS%EXTRA
      ELSE IF(ID.EQ.'IND') THEN
        IF(LEN.NE.THIS%NOCC) THEN
          CALL ERROR$MSG('LEN MUST BE NOCC FOR IND')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NOCC: ',THIS%NOCC)
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        IF(.NOT.ALLOCATED(THIS%IND)) THEN
          CALL ERROR$MSG('IND ARRAY NOT ALLOCATED')
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        VAL=THIS%IND
      ELSE IF(ID.EQ.'INDE') THEN
        IF(LEN.NE.THIS%NB2-THIS%NOCC) THEN
          CALL ERROR$MSG('LEN MUST BE NB2-NOCC FOR INDE')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NB2-NOCC: ',THIS%NB2-THIS%NOCC)
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        IF(.NOT.ALLOCATED(THIS%INDE)) THEN
          CALL ERROR$MSG('INDE ARRAY NOT ALLOCATED')
          CALL ERROR$STOP('OVERLAP$GETI4A')
        END IF
        VAL=THIS%INDE
      ELSE
        CALL ERROR$MSG('OVERLAP GETI4A ID NOT RECOGNIZED')
        CALL ERROR$CHVAL('ID: ',ID)
        CALL ERROR$STOP('OVERLAP$GETI4A')
      END IF
      RETURN
      END SUBROUTINE OVERLAP$GETI4A
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE OVERLAP$SETI4A(ID,LEN,VAL)  ! MARK: OVERLAP$SETI4A
!     **************************************************************************
!     ** SET INTEGER VALUE ARRAY IN SELECTED OVERLAP                         **
!     ** REQUIRES SELECTED OVERLAP                                           **
!     **************************************************************************
      USE OVERLAP_MODULE, ONLY: THIS,SELECTED
      IMPLICIT NONE
...
!     **************************************************************************
      IF(.NOT.SELECTED) THEN
        CALL ERROR$MSG('NO OVERLAP SELECTED')
        CALL ERROR$STOP('OVERLAP$SETI4A')
      END IF
      IF(ID.EQ.'MISSING') THEN
        IF(LEN.NE.THIS%NSWAPPED) THEN
          CALL ERROR$MSG('LEN MUST BE NSWAPPED FOR MISSING')
          CALL ERROR$MSG('SET NSWAPPED FIRST')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NSWAPPED: ',THIS%NSWAPPED)
          CALL ERROR$STOP('OVERLAP$SETI4A')
        END IF
        IF(.NOT.ALLOCATED(THIS%MISSING)) THEN
          ALLOCATE(THIS%MISSING(LEN))
        END IF
        THIS%MISSING=VAL
      ELSE IF(ID.EQ.'EXTRA') THEN
        IF(LEN.NE.THIS%NSWAPPED) THEN
      ...
      ELSE IF(ID.EQ.'INDE') THEN
        ...
        END IF
        IF(.NOT.ALLOCATED(THIS%INDE)) THEN
          ALLOCATE(THIS%INDE(LEN))
        END IF
        THIS%INDE=VAL
      ELSE
        CALL ERROR$MSG('OVERLAP SETI4A ID NOT RECOGNIZED')
        CALL ERROR$CHVAL('ID: ',ID)
        CALL ERROR$STOP('OVERLAP$SETI4A')
      END IF
      RETURN
      END SUBROUTINE OVERLAP$SETI4A
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
...
!     **************************************************************************
      USE OVERLAP_MODULE, ONLY: ADETSUM,THIS,SELECTED
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      REAL(8), INTENT(OUT) :: VAL
!     **************************************************************************
...
!     **************************************************************************
      USE OVERLAP_MODULE, ONLY: R2CORE
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      INTEGER(4), INTENT(IN) :: LEN
      REAL(8), INTENT(OUT) :: VAL(LEN)
!     **************************************************************************
      ...
      ELSE IF(ID.EQ.'R2CORE') THEN
        IF(LEN.NE.2) THEN
          CALL ERROR$MSG('LEN MUST BE 2 FOR R2CORE')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$CHVAL('ID: ',ID)
...
!     **************************************************************************
      ...
      ELSE IF(ID.EQ.'AUG') THEN
        ...
        IF(LEN.NE.NB2*NB1) THEN
          ...
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%AUG,(/NB2*NB1/))
      ELSE IF(ID.EQ.'A') THEN
        CALL CHECKNOCC
        IF(.NOT.ALLOCATED(THIS%A)) THEN
          IF(THIS%TOVERLAP) THEN
            ALLOCATE(THIS%A(NOCC,NOCC))
            ! CALCULATE A FROM PW AND AUG, CONSIDER MAXIMUM OVERLAP MAPPING IND
            DO I=1,NOCC
              DO J=1,NOCC
                THIS%A(I,J)=THIS%PW(THIS%IND(I),J)+THIS%AUG(THIS%IND(I),J)
              END DO
            END DO
          ELSE
        ...
        IF(LEN.NE.NOCC*NOCC) THEN
          ...
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%A,(/NOCC*NOCC/))
      ELSE IF(ID.EQ.'B') THEN
        CALL CHECKNOCC
        IF(.NOT.ALLOCATED(THIS%B)) THEN
          IF(THIS%TOVERLAP) THEN
            ALLOCATE(THIS%B(NOCC,NB1-NOCC))
            ! CALCULATE B FROM PW AND AUG, CONSIDER MAXIMUM OVERLAP MAPPING IND
            DO I=1,NOCC
              DO J=1,NB1-NOCC
                THIS%B(I,J)=THIS%PW(THIS%IND(I),NOCC+J)+THIS%AUG(THIS%IND(I),NOCC+J)
              END DO
            END DO
          ELSE
        ...
        IF(LEN.NE.NOCC*(NB1-NOCC)) THEN
          ...
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%B,(/NOCC*(NB1-NOCC)/))
      ELSE IF(ID.EQ.'C') THEN
        CALL CHECKNOCC
        IF(.NOT.ALLOCATED(THIS%C)) THEN
          IF(THIS%TOVERLAP) THEN
            ALLOCATE(THIS%C(NB2-NOCC,NOCC))
            ! CALCULATE C FROM PW AND AUG, CONSIDER MAXIMUM OVERLAP MAPPING INDE
            DO I=1,NB2-NOCC
              DO J=1,NOCC
                THIS%C(I,J)=THIS%PW(THIS%INDE(I),J)+THIS%AUG(THIS%INDE(I),J)
              END DO
            END DO
          ELSE
        ...
        IF(LEN.NE.(NB2-NOCC)*NOCC) THEN
          ...
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%C,(/(NB2-NOCC)*NOCC/))
      ELSE IF(ID.EQ.'D') THEN
        CALL CHECKNOCC
        IF(.NOT.ALLOCATED(THIS%D)) THEN
          IF(THIS%TOVERLAP) THEN
            ALLOCATE(THIS%D(NB2-NOCC,NB1-NOCC))
            ! CALCULATE D FROM PW AND AUG, CONSIDER MAXIMUM OVERLAP MAPPING INDE
            DO I=1,NB2-NOCC
              DO J=1,NB1-NOCC
                THIS%D(I,J)=THIS%PW(THIS%INDE(I),NOCC+J)+THIS%AUG(THIS%INDE(I),NOCC+J)
              END DO
            END DO
          ELSE
          ...
          END IF
        END IF
        IF(LEN.NE.(NB2-NOCC)*(NB1-NOCC)) THEN
          ...
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%D,(/(NB2-NOCC)*(NB1-NOCC)/))
      ELSE IF(ID.EQ.'AINV') THEN
        ...
        IF(.NOT.ALLOCATED(THIS%AINV)) THEN
          IF(.NOT.ALLOCATED(THIS%A)) THEN
            ...
            END IF
          END IF
          ALLOCATE(THIS%AINV(NOCC,NOCC))
          ! CALCULATE AINV FROM A
          CALL LIB$INVERTC8(NOCC,THIS%A,THIS%AINV)
        END IF
        IF(LEN.NE.NOCC*NOCC) THEN
          CALL ERROR$MSG('LEN MUST BE EQUAL TO NOCC*NOCC')
          CALL ERROR$I4VAL('LEN: ',LEN)
      ...
      ELSE IF(ID.EQ.'DIPOLE') THEN
        ...
        ALLOCATE(WORK(3,NB2))
        DO I=1,NOCC
          WORK(:,I)=THIS%DIPOLE(:,THIS%IND(I))
        END DO
        DO I=NOCC+1,NB2
          WORK(:,I)=THIS%DIPOLE(:,THIS%INDE(I-NOCC))
        END DO
        VAL(:)=RESHAPE(WORK,(/3*NB2/))
        DEALLOCATE(WORK)
      ELSE IF(ID.EQ.'DIPOLEGS') THEN
        IF(.NOT.ALLOCATED(THIS%DIPOLEGS)) THEN
          CALL ERROR$MSG('DIPOLEGS NOT ALLOCATED')
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        IF(LEN.NE.3*NB1) THEN
          CALL ERROR$MSG('LEN MUST BE EQUAL TO 3*NB1')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NB1: ',NB1)
          CALL ERROR$CHVAL('ID: ',ID)
          CALL ERROR$STOP('OVERLAP$GETC8A')
        END IF
        VAL(:)=RESHAPE(THIS%DIPOLEGS,(/3*NB1/))
      ELSE
...
!     **************************************************************************
      ...
      ELSE IF(ID.EQ.'DIPOLEGS') THEN
        IF(THIS%NB1.EQ.-1) THEN
          CALL ERROR$MSG('NB1 NOT SET')
          CALL ERROR$STOP('OVERLAP$SETC8A')
        END IF
        IF(.NOT.ALLOCATED(THIS%DIPOLEGS)) THEN
          ALLOCATE(THIS%DIPOLEGS(3,THIS%NB1))
        END IF
        IF(LEN.NE.3*THIS%NB1) THEN
          CALL ERROR$MSG('LEN MUST BE EQUAL TO 3*NB1')
          CALL ERROR$I4VAL('LEN: ',LEN)
          CALL ERROR$I4VAL('NB1: ',THIS%NB1)
          CALL ERROR$CHVAL('ID: ',ID)
          CALL ERROR$STOP('OVERLAP$SETC8A')
        END IF
        THIS%DIPOLEGS(:,:)=RESHAPE(VAL,(/3,THIS%NB1/))
      ELSE
...
!     **************************************************************************
      ...
      DO IKPT=1,NKPTG
        DO ISPIN=1,NSPING
          ...
          CALL MPE$SENDRECEIVE('~',WTASK,RTASK,DIPOLEGS)
          IF(THISTASK.EQ.RTASK) THEN
            ! NB1,NB2,NOCC
            WRITE(NFIL)NB1,NB2,NOCC
            ! PW,AUG,DIPOLE,DIPOLEGS
            WRITE(NFIL)PW,AUG,DIPOLE,DIPOLEGS
          END IF
          DEALLOCATE(PW)
          DEALLOCATE(AUG)
          DEALLOCATE(DIPOLE)
        ...
        ENDDO
      ENDDO
      IF(THISTASK.EQ.RTASK) WRITE(NFIL) R2CORE
                          CALL TRACE$POP
      RETURN
...
!     **************************************************************************
      ...
      CALL OVERLAP$SETR8A('R2CORE',2,R2CORE)
                          CALL TRACE$POP
      RETURN
      END SUBROUTINE OVERLAP_READOVERLAP
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE OVERLAP_MAXOVERLAP(IKPT,ISPIN)  ! MARK: OVERLAP_MAXOVERLAP
!     **************************************************************************
!     ** FIND SET OF EXCITED STATES YIELDING HIGH OVERLAP WITH GROUND STATE   **
!     ** USES QR DECOMPOSITION TO FIND MAXIMAL OVERLAP BETWEEN TWO SETS OF    **
!     ** STATES (GROUND AND EXCITED)                                          **
!     **************************************************************************        
      USE OVERLAP_MODULE, ONLY: INITIALIZED,SELECTED,THIS
      IMPLICIT NONE
      INTEGER(4), INTENT(IN) :: IKPT,ISPIN
      LOGICAL(4), PARAMETER :: DEBUG=.FALSE.
      LOGICAL(4) :: MAXOVERLAP
      INTEGER(4) :: NB1,NB2,NOCC
      INTEGER(4) :: I,J,KEY
      INTEGER(4) :: NTASKS,THISTASK,RTASK,WTASK
      ...
      ! REQUIRED FOR ZGEQP3
      COMPLEX(8), ALLOCATABLE :: TAU(:) ! (NOCC)
      COMPLEX(8), ALLOCATABLE :: AT(:,:) ! (NOCC,NB2)
      COMPLEX(8), ALLOCATABLE :: WORK(:) ! (LWORK)
      INTEGER(4), ALLOCATABLE :: JPVT(:) ! (NB2)
      REAL(8), ALLOCATABLE :: RWORK(:) ! (2*NB2)
      INTEGER(4) :: LWORK,INFO

      INTEGER(4) :: NSWAPPED
      INTEGER(4), ALLOCATABLE :: MISSING(:)
      INTEGER(4), ALLOCATABLE :: ADDITIONAL(:)
      REAL(8), ALLOCATABLE :: EIG(:)
!     **************************************************************************
! TODO: MOVE A ZGEQP3 INTERFACE TO PAW_LIBRARY.F90
      EXTERNAL ZGEQP3
                          CALL TRACE$PUSH('OVERLAP_MAXOVERLAP')
      IF(.NOT.INITIALIZED) THEN
        CALL ERROR$MSG('OVERLAP NOT INITIALIZED')
        CALL ERROR$STOP('OVERLAP_MAXOVERLAP')
      END IF
      IF(.NOT.SELECTED) THEN
        CALL ERROR$MSG('NO OVERLAP SELECTED')
        CALL ERROR$STOP('OVERLAP_MAXOVERLAP')
      END IF
      ! IF ALREADY RUN BEFORE, RETURN
      ...
      NB2=THIS%NB2
      NOCC=THIS%NOCC
      ALLOCATE(THIS%IND(NOCC))
      ALLOCATE(THIS%INDE(NB2-NOCC))

      ! IF MAXOVERLAP IS FALSE, THEN RETURN INDICES 1:NOCC AND NOCC+1:NB2
      CALL XCNTL$GETL4('MAXOVERLAP',MAXOVERLAP)
      IF(.NOT.MAXOVERLAP) THEN
        DO I=1,NOCC
          THIS%IND(I)=I
        ENDDO
        DO I=1,NB2-NOCC
      ...
      ALLOCATE(OVL(NB2,NOCC))
      OVL(:,:)=THIS%PW(:,1:NOCC)+THIS%AUG(:,1:NOCC)

      ALLOCATE(TAU(NOCC))
      ALLOCATE(AT(NOCC,NB2))
      ALLOCATE(JPVT(NB2))
      ALLOCATE(RWORK(2*NB2))
      ALLOCATE(WORK(1))
      AT=TRANSPOSE(OVL)
      JPVT(:)=0
      LWORK=-1
      CALL ZGEQP3(NOCC,NB2,AT,NOCC,JPVT,TAU,WORK,LWORK,RWORK,INFO)
      IF(INFO.NE.0) THEN
        CALL ERROR$MSG('ZGEQP3 FAILED')
        CALL ERROR$I4VAL('INFO: ',INFO)
        CALL ERROR$STOP('OVERLAP_MAXOVERLAP')
      END IF
      LWORK=MAX(1,NINT(REAL(WORK(1),KIND=8)))
      DEALLOCATE(WORK)
      ALLOCATE(WORK(LWORK))
      CALL ZGEQP3(NOCC,NB2,AT,NOCC,JPVT,TAU,WORK,LWORK,RWORK,INFO)
      IF(INFO.NE.0) THEN
        ...
        CALL ERROR$STOP('OVERLAP_MAXOVERLAP')
      END IF
      ! JPVT CONTAINS THE INDICES OF THE COLUMNS OF AT THAT FORM THE MAXIMAL OVERLAP
      ! NEED TO STORE THE FIRST NOCC INDICES IN THIS%IND
      THIS%IND(:)=JPVT(1:NOCC)
      ...
      IF(THIS%NSWAPPED.GT.0) THEN
        ALLOCATE(THIS%MISSING(THIS%NSWAPPED))
        ALLOCATE(THIS%EXTRA(THIS%NSWAPPED))
        THIS%MISSING(:)=PACK([(I,I=1,NOCC)],[(.NOT.ANY(THIS%IND == I),I=1,NOCC)])
        THIS%EXTRA(:)=PACK(THIS%IND,[( (THIS%IND(I) > NOCC),I=1,NOCC)])
        ...
        THIS%ESWAP=0.D0
        DO I=1,THIS%NSWAPPED
          THIS%ESWAP=THIS%ESWAP+EIG(THIS%EXTRA(I))-EIG(THIS%MISSING(I))
        ENDDO
        DEALLOCATE(EIG)
      ELSE
        THIS%ESWAP=0.D0
      ...
      IF(DEBUG) THEN
        ALLOCATE(AT(NOCC,NOCC))
        DO I=1,NOCC
          DO J=1,NOCC
            AT(I,J)=OVL(THIS%IND(I),J)
          ENDDO
        ENDDO
        CALL LIB$DETC8(NOCC,OVL(1:NOCC,1:NOCC),CVAR)
PRINT *, 'OVERLAP DEFAULT: ', abs(CVAR)**2
        CALL LIB$DETC8(NOCC,AT,CVAR)
PRINT *, 'OVERLAP MAXOVERLAP: ', abs(CVAR)**2 
        DEALLOCATE(AT)
      END IF
...
!     **************************************************************************
      USE STRINGS_MODULE
      IMPLICIT NONE
      CHARACTER(8), PARAMETER :: FLAG='20260401' ! FLAG TO CATCH INCONSISTENT FILES
      LOGICAL(4) :: TOVERLAP
      INTEGER(4) :: NFIL
      CHARACTER(256) :: FILENAME
      INTEGER(4) :: NTASKS,THISTASK,RTASK
...
!     **************************************************************************
      ...
      IF(THISTASK.EQ.1) THEN
        ...
        CALL FILEHANDLER$UNIT('OVERLAP',NFIL)
        READ(NFIL)FLAG_READ
        IF(FLAG_READ.NE.FLAG) THEN
          CALL ERROR$MSG('FLAG IN OVERLAP FILE DOES NOT MATCH EXPECTED VALUE')
          CALL ERROR$MSG('FILE MAY BE INCONSISTENT WITH CURRENT VERSION OF CODE')
          CALL ERROR$MSG('OR FILE MAY BE CORRUPTED')
          CALL ERROR$MSG('PLEASE START FROM SIMULATION !ANALYSE!OVL FILES INSTEAD')
          CALL ERROR$CHVAL('FLAG_READ: ',FLAG_READ)
          CALL ERROR$CHVAL('EXPECTED FLAG: ',FLAG)
          CALL ERROR$STOP('XRAY$READOVERLAP')
        END IF
      END IF
...
!     **************************************************************************
      ...
      INTEGER(4) :: NB1,NB2,NOCC
      INTEGER(4) :: NB2TMP ! (NB2) VALUE DISTRIBUTED TO ALL TASKS
      INTEGER(4) :: NOCCTMP ! (NOCC) VALUE DISTRIBUTED TO ALL TASKS
      INTEGER(4) :: NKPT,NSPIN
      INTEGER(4) :: IKPT,ISPIN
      INTEGER(4) :: NTASKS,THISTASK,WTASK,RTASK
      ...
      REAL(8) :: SVAR
      LOGICAL(4) :: TNORMALIZE
      INTEGER(4), ALLOCATABLE :: NSWAPPED(:,:)
      REAL(8), ALLOCATABLE :: ESWAP(:,:)
      INTEGER(4), ALLOCATABLE :: SWAPPED(:)
...
!     **************************************************************************
      ...
      ADET=(0.D0,0.D0)
      ALLOCATE(NSWAPPED(NKPT,NSPIN))
      ALLOCATE(ESWAP(NKPT,NSPIN))
      NSWAPPED=0
      ESWAP=0.D0
      CALL SIMULATION$UNSELECT
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ...
          CALL OVERLAP_MAXOVERLAP(IKPT,ISPIN)
          CALL OVERLAP$GETI4('NSWAPPED',NSWAPPED(IKPT,ISPIN))
          CALL OVERLAP$GETR8('ESWAP',ESWAP(IKPT,ISPIN))
      ...
      CALL MPE$COMBINE('~','+',NSWAPPED)
      CALL MPE$COMBINE('~','+',ESWAP)
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          CALL KSMAP$WORKTASK(IKPT,ISPIN,WTASK)
          CALL OVERLAP$SELECT(IKPT,ISPIN)
          ! NB2 AND NOCC ARE ONLY KNOWN TO THE OWNING TASK AND THE READ
          ! TASK (THEY WERE NEVER SET ON OTHER TASKS, I.E. THIS%NB2=-1
          ! AND THIS%NOCC=-1). DISTRIBUTE THEM TO ALL TASKS SO THAT THE
          ! COLLECTIVES BELOW USE IDENTICAL ARRAY SIZES ON EVERY TASK.
          IF(THISTASK.EQ.WTASK) THEN
            CALL OVERLAP$GETI4('NB2',NB2)
            CALL OVERLAP$GETI4('NOCC',NOCC)
          END IF
          CALL MPE$BROADCAST('~',WTASK,NB2)
          CALL MPE$BROADCAST('~',WTASK,NOCC)
          ! STORE THE DISTRIBUTED VALUES ONLY WHERE THEY ARE NOT KNOWN YET
          CALL OVERLAP$GETI4('NB2',NB2TMP)
          IF(NB2TMP.EQ.-1) CALL OVERLAP$SETI4('NB2',NB2)
          CALL OVERLAP$GETI4('NOCC',NOCCTMP)
          IF(NOCCTMP.EQ.-1) CALL OVERLAP$SETI4('NOCC',NOCC)
          CALL OVERLAP$SETC8('ADET',ADET(IKPT,ISPIN))
          CALL OVERLAP$SETI4('NSWAPPED',NSWAPPED(IKPT,ISPIN))
          CALL OVERLAP$SETR8('ESWAP',ESWAP(IKPT,ISPIN))
          IF(NSWAPPED(IKPT,ISPIN).GT.0) THEN
            ALLOCATE(SWAPPED(NSWAPPED(IKPT,ISPIN)))
            SWAPPED=0
            IF(THISTASK.EQ.WTASK) THEN
              CALL OVERLAP$GETI4A('MISSING',NSWAPPED(IKPT,ISPIN),SWAPPED)
            END IF
            CALL MPE$COMBINE('~','+',SWAPPED)
            CALL OVERLAP$SETI4A('MISSING',NSWAPPED(IKPT,ISPIN),SWAPPED)
            SWAPPED=0
            IF(THISTASK.EQ.WTASK) THEN
              CALL OVERLAP$GETI4A('EXTRA',NSWAPPED(IKPT,ISPIN),SWAPPED)
            END IF
            CALL MPE$COMBINE('~','+',SWAPPED)
            CALL OVERLAP$SETI4A('EXTRA',NSWAPPED(IKPT,ISPIN),SWAPPED)
            DEALLOCATE(SWAPPED)
          END IF
          ALLOCATE(IND(NOCC))
          IND=0
          IF(THISTASK.EQ.WTASK) THEN
            CALL OVERLAP$GETI4A('IND',NOCC,IND)
          END IF
          CALL MPE$COMBINE('~','+',IND)
          CALL OVERLAP$SETI4A('IND',NOCC,IND)
          DEALLOCATE(IND)
          ALLOCATE(INDE(NB2-NOCC))
          INDE=0
          IF(THISTASK.EQ.WTASK) THEN
            CALL OVERLAP$GETI4A('INDE',NB2-NOCC,INDE)
          END IF
          CALL MPE$COMBINE('~','+',INDE)
          CALL OVERLAP$SETI4A('INDE',NB2-NOCC,INDE)
          DEALLOCATE(INDE)
          CALL OVERLAP$UNSELECT
        ENDDO ! END ISPIN
      ENDDO ! END IKPT
...
! WARNING: CHOOSE ROUT=R(NR-3) AS DONE ON PAW_SETUPS.F90
          CALL RADIAL$INTEGRATE(GID,NR,WORK,WORK1)
          CALL RADIAL$VALUE(GID,NR,WORK1,ROUT,RADINT(LN))
        ENDDO ! END LN
        ! LOOP OVER K POINTS
        DO IKPT=1,NKPT
          ! LOOP OVER SPIN
          DO ISPIN=1,NSPIN
            ...
            ENDDO ! END IB
            IF(ISIM.EQ.1) THEN
              CALL OVERLAP$SETC8A('DIPOLEGS',3*NB,DIPOLE)
            ELSE IF(ISIM.EQ.2) THEN
              CALL OVERLAP$SETC8A('DIPOLE',3*NB,DIPOLE)
            ELSE
              CALL ERROR$MSG('INVALID SIMULATION INDEX')
              CALL ERROR$I4VAL('ISIM',ISIM)
              CALL ERROR$STOP('XRAY_DIPOLEMATRIX')
            END IF
            CALL OVERLAP$UNSELECT
            DEALLOCATE(DIPOLE)
        ...
        ! TODO: <C|R^2|C>
        WORK=AEPSI(:,IBCORE)*R**4*AEPSI(:,IBCORE)
        CALL RADIAL$INTEGRATE(GID,NR,WORK,WORK1)
        CALL RADIAL$VALUE(GID,NR,WORK1,ROUT,R2CORE(ISIM))
      ...
      END DO ! END ISIM
      R2CORE=R2CORE/3.D0
      CALL OVERLAP$SETR8A('R2CORE',NSIM,R2CORE)
...
! WARNING: CHOOSE ROUT=R(NR-3) AS DONE ON PAW_SETUPS.F90
            CALL RADIAL$INTEGRATE(GID1,NR1,AUX,AUX1)
            CALL RADIAL$VALUE(GID1,NR1,AUX1,ROUT,S(IAT1,LN1,LN2))
          ENDDO ! END LN2
          DEALLOCATE(PSPHI2)
          DEALLOCATE(AEPHI2)
...
! WARNING: CHOOSE ROUT=R(NR-3) AS DONE ON PAW_SETUPS.F90
      ...
      CALL RADIAL$INTEGRATE(GID_G,NR_G,WORK,WORK1)
      CALL RADIAL$VALUE(GID_G,NR_G,WORK1,ROUT,SVAR)
      ...
      CALL RADIAL$INTEGRATE(GID_E,NR_E,WORK,WORK1)
      CALL RADIAL$VALUE(GID_E,NR_E,WORK1,ROUT,SVAR)
...
!     **************************************************************************
      ...
      DO IORB=1,NAUXORB
        ...
        ! CHECK IF BAND IS OUT OF RANGE
        IF(BAND.LT.1.OR.BAND.GT.NB2) THEN
          CALL ERROR$MSG('BAND OUT OF RANGE')
          CALL ERROR$I4VAL('B',BAND)
          CALL ERROR$I4VAL('NB2',NB2)
          CALL ERROR$STOP('AUXORB$CALCULATE')
        END IF
        ! CHECK IF BAND IS IN IND E.G. OCCUPIED
        IF(ANY(IND==BAND)) THEN
          CALL ERROR$MSG('BAND IN OCCUPIED ORBITALS')
          CALL ERROR$I4VAL('B',BAND)
          IF(MAXOVERLAP) THEN
            CALL ERROR$MSG('MAXOVERLAP ACTIVE - OCCUPIED ORBITALS MIGHT HAVE SWAPPED')
          END IF
          CALL ERROR$STOP('AUXORB$CALCULATE')
        END IF
        THIS%NOCC=NOCC
        ! FIND INDEX OF UNOCCUPIED BAND IN INDE ARRAY
        DO IEMP=1,NB2-NOCC
          IF(INDE(IEMP).EQ.BAND) EXIT
        ENDDO
        ALLOCATE(KMAT(NB2-NOCC,NOCC))
        CALL OVERLAP$K(NB2-NOCC,NOCC,KMAT)
        ALLOCATE(DIPOLE(3,NB2))
...
!     ==========================================================================
!     ==========================================================================
! WARNING: XASDOS DOES NOT RECOGNISE THE CORRECTNESS OF THE .DOSRAW INPUT FILE
!          1. INITIAL PAW_XRAY RUN CREATES ROOT_XAS.PDOS FILE
!          2. RUN PAW_DOS ON THE ROOT_XAS.PDOS FILE TO CREATE ROOT_XAS.DOSRAW FILE
!          3. USE THE ROOT_XAS.DOSRAW FILE IN XASDOS
!          ROOT_XAS.PDOS IS SENSITIVE TO MAXOVERLAP AND REORDERING OF ORBITALS
!          FOR CORRECT RESULTS ALL FILES NEED TO BE CREATED WITH THE SAME SETTING
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
...
!       -IKPT IS CONNECTED VIA INVERSION SYMMETRY
              SVAR(:)=XAS*SET(INDE(IEMP),IKPT,ISPIN,:)*DE
...
!          OF THE XAS SPECTRUM
    ...
    !           I1=EWGHT(INDE(IEMP),IKPT,ISPIN)%I1
    !           I2=EWGHT(INDE(IEMP),IKPT,ISPIN)%I2
    !           DO IE=I1,I2
    !             DOS(IE,ISPIN,:,ISPEC)=DOS(IE,ISPIN,:,ISPEC)+ &
    !  &                                EWGHT(INDE(IEMP),IKPT,ISPIN)%WGHT(IE)*SVAR(:)
    !           ENDDO
...
!     **************************************************************************
      ...
      REAL(8) :: NORM
      COMPLEX(8), ALLOCATABLE :: KMAT(:,:)
      INTEGER(4), ALLOCATABLE :: IND(:) ! (NOCC) DETERMINANT OCCUPIED BANDS
      INTEGER(4), ALLOCATABLE :: INDE(:) ! (NB-NOCC) DETERMINANT EMPTY BANDS
      CHARACTER(4) :: ID
      INTEGER(4) :: NTASKS,THISTASK,RTASK,WTASK
!     **************************************************************************
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ...
          IF(THISTASK.EQ.RTASK) THEN
            ! GET THE MAXIMUM-OVERLAP BAND MAPPINGS (DISTRIBUTED TO ALL
            ! TASKS IN XRAY$OVERLAP): IND=OCCUPIED, INDE=EMPTY STATES OF
            ! THE DETERMINANT IN ORIGINAL BAND ORDER
            ALLOCATE(IND(NOCC))
            ALLOCATE(INDE(NB-NOCC))
            CALL OVERLAP$SELECT(IKPT,ISPIN)
            CALL OVERLAP$GETI4A('IND',NOCC,IND)
            ...
            ! THE AUXILIARY ORBITAL OF THE EMPTY STATE IEMP IS STORED AT THE
            ! ORIGINAL BAND POSITION INDE(IEMP) AND THE OCCUPIED SET OF THE
            ! DETERMINANT IS IND: THIS IS THE BAND INDEXING EXPECTED BY
            ! XASDOS$CALCULATE, WHICH WEIGHTS SET(INDE(IEMP)) WITH THE XAS
            ! INTENSITY OF THE AUXILIARY ORBITAL
! WARNING: CAREFUL WITH USING THE AUXILIARY ORBITALS FOR FURTHER CALCULATIONS
            ! LOOP OVER EMPTY STATES OF THE DETERMINANT (F=INDE(IEMP))
            DO IEMP=1,NB-NOCC
              IEMPTOT=INDE(IEMP)
              NORM=1.D0
              ! LOOP OVER OCCUPIED STATES OF THE DETERMINANT (I=IND(IB))
              DO IB=1,NOCC
                PROJREORDERED(:,:,IEMPTOT)=PROJREORDERED(:,:,IEMPTOT)- &
                &               KMAT(IEMP,IB)*PROJREORDERED(:,:,IND(IB))
     ...
     &                                     ADET*ADETOP
              END IF
              ! EMPTY STATES OF THE DETERMINANT ARE UNOCCUPIED
              OCC(IEMPTOT)=0.D0
            ENDDO
            ! OCCUPIED STATES OF THE DETERMINANT ARE FULLY OCCUPIED
            ! (OCC CARRIES THE K-POINT WEIGHT; MAXIMUM OCCUPATION IS 2 FOR
            ! NON-SPIN-POLARIZED AND 1 FOR SPIN-POLARIZED CALCULATIONS)
            DO IB=1,NOCC
              OCC(IND(IB))=WKPT(IKPT)*REAL(MOD(NSPIN,2)+1,KIND=8)
            ENDDO
            DEALLOCATE(IND)
            DEALLOCATE(INDE)
            ! WRITE PDOS DATA
...
!     **************************************************************************
      ...
      WRITE(NFIL,'(80("#"))')
      WRITE(NFIL,FMT='(A)')'WARNING: XASDOS DOES NOT RECOGNISE THE'
      WRITE(NFIL,FMT='(A)')'   CORRECTNESS OF THE .DOSRAW INPUT FILE'
      WRITE(NFIL,FMT='(A)')'   1. INITIAL PAW_XRAY RUN CREATES ROOT_XAS.PDOS FILE'
      WRITE(NFIL,FMT='(A)')'   2. RUN PAW_DOS ON THE ROOT_XAS.PDOS FILE TO CREATE ROOT_XAS.DOSRAW FILE'
      WRITE(NFIL,FMT='(A)')'   3. USE THE ROOT_XAS.DOSRAW FILE IN XASDOS'
      WRITE(NFIL,FMT='(A)')'   ROOT_XAS.PDOS IS SENSITIVE TO MAXOVERLAP AND REORDERING OF ORBITALS'
      WRITE(NFIL,FMT='(A)')'   FOR CORRECT RESULTS ALL FILES NEED TO BE CREATED WITH THE SAME SETTING'
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE ROTMATFROMN(N1,N2,R) ! MARK: ROTMATFROMN
!     **************************************************************************
!     **  CALCULATE ROTATION MATRIX R TO ROTATE NORMAL VECTOR N1 TO N2     **
!     **  USING RODRIGUES' ROTATION FORMULA                               **
!     **************************************************************************
      IMPLICIT NONE
      REAL(8), INTENT(IN) :: N1(3)  ! INITIAL NORMAL VECTOR
      REAL(8), INTENT(IN) :: N2(3)  ! FINAL NORMAL VECTOR
...
!     **************************************************************************
      ...
      N01=N1/NORM2(N1)
      N02=N2/NORM2(N2)
      CALL CROSS_PROD(N01,N02,AXIS)
      SIN_A=NORM2(AXIS)
      COS_A=DOT_PRODUCT(N01,N02)
      IF(SIN_A.LT.1.D-8) THEN
        R=EYE
        RETURN
      END IF
      AXIS=AXIS/SIN_A
      ...
      K(3,2)=AXIS(1)
      K2=MATMUL(K,K)
      R=EYE+K*SIN_A+K2*(1.D0-COS_A)
      END SUBROUTINE ROTMATFROMN
