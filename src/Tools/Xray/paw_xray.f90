...
!     **************************************************************************
      ...
      REAL(8), ALLOCATABLE :: XK(:,:) ! (3,NKPT) IRR. K-POINTS IN REL. COORD.
      REAL(8), ALLOCATABLE :: XKTOT(:,:) ! (3,NKPTTOT) K-POINTS IN REL. COORD.
...
!     **************************************************************************
      ...
      TYPE(REAL_CONTAINER), ALLOCATABLE :: ERAW(:,:) ! (NKPT,NSPIN)%V(NB2-NOCC) RAW ENERGY
      TYPE(REAL_CONTAINER), ALLOCATABLE :: SRAW(:,:) ! (NKPT,NSPIN)%V(NB2-NOCC) RAW SPECTRUM
...
!     ...1.........2.........3.........4.........5.........6.........7.........8
      ...
      COMPLEX(8), ALLOCATABLE :: AMPL(:,:) ! (3,NOCC+1) CONTRIBUTION TO AMPLITUDE
      COMPLEX(8), ALLOCATABLE :: DIPOLE(:,:) ! (3,NOCC+1) DIPOLE MATRIX ELEMENTS
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      MODULE XASDOS_MODULE  ! MARK: XASDOS_MODULE
      USE BRILLOUIN_MODULE, ONLY: EWGHT_TYPE
      TYPE(EWGHT_TYPE), ALLOCATABLE :: EWGHT(:,:,:) ! (NB,NKPT,NSPIN)
      REAL(8), ALLOCATABLE :: WGHT(:,:,:) ! (NB,NKPT,NSPIN)

      REAL(8) :: EF ! FERMI LEVEL

      CHARACTER(32) :: FILE ! FILENAME FOR DOSRAW
      INTEGER(4) :: NBB ! NUMBER OF BANDS
      INTEGER(4) :: NKPT ! NUMBER OF KPOINTS
      INTEGER(4) :: NSET ! NUMBER OF SETS

      REAL(8), ALLOCATABLE :: EIGVAL(:,:) ! (NBB,NKPT) EIGENVALUES
      REAL(8), ALLOCATABLE :: EIG(:,:,:) ! (NB,NKPT,NSPIN)
      REAL(8), ALLOCATABLE :: SETVAL(:,:,:,:) ! (NBB,NKPT,2,NSET) PROJECTED DOS
...
!     ==========================================================================
      ...
      ! READ DOS FILE AND SAVE DATA ON READ TASK ONLY
      CALL XASDOS$READ
      ! VALIDATE XASDOS DATA AGAINST SIMULATION DATA
      ! REFORMAT INTO COMPATIBLE ARRAYS
      CALL XASDOS$VALIDATE
...
!     **************************************************************************
      ...
      IF(TCHK) THEN
        CALL LINKEDLIST$GET(LL_CNTL,'RAW',1,TCHK)
      ...
      CALL LINKEDLIST$SELECT(LL_CNTL,'XCNTL')
      CALL LINKEDLIST$SELECT(LL_CNTL,'XAS')
      CALL LINKEDLIST$EXISTL(LL_CNTL,'DOS',1,TCHK)
      CALL XASDOS$SETL4('ACTIVE',TCHK)
...
!     **************************************************************************
      ...
      IF(ALLOCATED(THIS%TOTOTKPT)) THEN
        ...
        CALL ERROR$STOP('SIMULATION$KPTMAP')
      END IF
      NKPTTOT=THIS%NKPTTOT
      NKPT=THIS%NKPT
      ! ALLOCATE K-POINT MAPPING
      ALLOCATE(THIS%TOIRRKPT(NKPTTOT))
      ALLOCATE(THIS%TIRRKPT(NKPTTOT))
      ALLOCATE(THIS%TOTOTKPT(NKPT))
      ! ALLOCATE TOTAL K-POINT SET
      ALLOCATE(THIS%XKTOT(3,NKPTTOT))
      ...
      DO IKPT=1,NKPT
        ...
        ! IF TRUE: NO K POINT -K NEEDS TO BE CALCULATED
        ! IF FALSE: -K POINT NEEDS TO BE CALCULATED
        IF(THIS%TINVARR(IKPT)) THEN ! NEGATIVE K-POINT IS NOT NEEDED
          THIS%TOIRRKPT(IKPTTOT)=IKPT
          ! IKPTTOT IS IRR. KPT
          THIS%TIRRKPT(IKPTTOT)=.TRUE.
          ! COPY K-POINT TO TOTAL K-POINT SET
          THIS%XKTOT(:,IKPTTOT)=XK(:)
        ELSE ! NEGATIVE K-POINT IS NEEDED
          THIS%TOIRRKPT(IKPTTOT)=IKPT
          ! IKPTTOT IS IRR. KPT
          THIS%TIRRKPT(IKPTTOT)=.TRUE.
          ! COPY K-POINT TO TOTAL K-POINT SET
          ...
          ! COPY NEGATIVE K-POINT TO TOTAL K-POINT SET
          THIS%XKTOT(:,IKPTTOT)=MODULO(-XK,1.D0)
        END IF
      ENDDO
                          CALL TRACE$POP
      RETURN
...
!     **************************************************************************
     ...
     &                                 THIS%WKPT(IKPT),THIS%TINVARR(IKPT)
      ...
      WRITE(NFIL,FMT='(A8,A3,24X,A8,A9,9X,A9,A10)')'IKPTTOT','XK','IKPT', &
     &          'IRR. KPT','IKPT','IKPTTOT'
      DO IKPT=1,THIS%NKPTTOT
        IF(IKPT.LE.THIS%NKPT) THEN
          WRITE(NFIL,FMT='(I8,3F9.6," ->",I5,L9,9X,I9," ->",I7)')IKPT,&
     &           THIS%XKTOT(:,IKPT),THIS%TOIRRKPT(IKPT),THIS%TIRRKPT(IKPT), &
     &           IKPT,THIS%TOTOTKPT(IKPT)
        ELSE
          WRITE(NFIL,FMT='(I8,3F9.6," ->",I5,L9)')IKPT, &
     &           THIS%XKTOT(:,IKPT),THIS%TOIRRKPT(IKPT),THIS%TIRRKPT(IKPT)
        END IF
      ENDDO
      FLUSH(NFIL)
...
!     **************************************************************************
      ...
      ELSE
        ...
        ELSE IF(ID.EQ.'XKTOT') THEN
          IF(LEN.NE.THIS%NKPTTOT*3) THEN
            CALL ERROR$MSG('LENGTH OF XKTOT ARRAY MUST BE EQUAL TO NKPTTOT*3')
            CALL ERROR$I4VAL('LENGTH OF XKTOT: ',LEN)
            CALL ERROR$I4VAL('NKPTTOT: ',THIS%NKPTTOT)
            CALL ERROR$STOP('SIMULATION$GETR8A')
          END IF
          VAL(:)=RESHAPE(THIS%XKTOT,(/3*THIS%NKPTTOT/))
        ELSE IF(ID.EQ.'WKPT') THEN
          IF(LEN.NE.THIS%NKPT) THEN
            CALL ERROR$MSG('LENGTH OF WKPT ARRAY MUST BE EQUAL TO NKPT')
            CALL ERROR$I4VAL('LENGTH OF WKPT: ',LEN)
...
!     **************************************************************************
     ...
     &                        'SYMBOL','R(ASA)'
      ...
      DO ISP=1,THISNSP
        ! IF NO ATOM WITH THIS SPECIES IS PRESENT
        IF(THIS(ISP)%SYMBOL.EQ.'XX') THEN
          WRITE(NFIL,FMT=FORMAT)ISP,THIS(ISP)%GID,THIS(ISP)%ECORE, &
     &                        THIS(ISP)%LNX,THIS(ISP)%L(:), 'NOT USED', &
     &                        0.0D0
        ELSE
        WRITE(NFIL,FMT=FORMAT)ISP,THIS(ISP)%GID,THIS(ISP)%ECORE, &
...
!     **************************************************************************
      ...
      ! IF SPECIES IS NOT USED FOR ATOMS, SET SYMBOL TO 'XX' AND RASA TO RADIUS
      ! OF EMPTY ATOM
      IF(SYMBOL.EQ.'  ') THEN
        THIS(ISP)%SYMBOL='XX'
        CALL PERIODICTABLE$GET('0_','R(ASA)',THIS(ISP)%RASA)
      ELSE
...
!     **************************************************************************
      ...
      DO ISPEC=1,NSPEC
        ...
        THIS%SPECTRUM(:,:,:)=0.D0
        ALLOCATE(THIS%ERAW(NKPT,NSPIN))
        ALLOCATE(THIS%SRAW(NKPT,NSPIN))
        DO I=1,NE
      ...
      DO IKPT=1,NKPT
        ...
        DO ISPIN=1,NSPIN
          ...
          DO ISPEC=1,NSPEC
            CALL XAS$ISELECT(ISPEC)
            ALLOCATE(THIS%ERAW(IKPT,ISPIN)%V(NB2-NOCC))
            ALLOCATE(THIS%SRAW(IKPT,ISPIN)%V(NB2-NOCC))
            CALL XAS$UNSELECT
          ENDDO
          ...
          DO IEMP=1,NB2-NOCC
            ...
            DO ISPEC=1,NSPEC
              ...
              ! SAVE RAW SPECTRUM
              THIS%ERAW(IKPT,ISPIN)%V(IEMP)=EDIFF
              THIS%SRAW(IKPT,ISPIN)%V(IEMP)=SIGMA
              IF(TNEGATIVEKPT) THEN
                ...
                CALL MAPGRID(EDIFF,SIGMA,NE,THIS%SPECTRUM(IKPT,ISPIN,:),EMIN,DE)
                ! SAVE RAW SPECTRUM
                THIS%SRAW(IKPT,ISPIN)%V(IEMP)=THIS%SRAW(IKPT,ISPIN)%V(IEMP)+SIGMA
              END IF
              CALL XAS$UNSELECT
            ENDDO ! END ISPEC
          ENDDO ! END IEMP
      ...
      DO ISPEC=1,NSPEC
        ...
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            ...
            END IF  
            ! SEND RAW SPECTRUM TO READ TASK
            CALL KSMAP$WORKTASK(IKPT,ISPIN,WTASK)
            ...
            CALL OVERLAP$GETI4('NOCC',NOCC)
            CALL OVERLAP$UNSELECT
            ! ALLOCATE RAW SPECTRUM ON READTASK
            IF(THISTASK.EQ.RTASK) THEN
              IF(.NOT.ALLOCATED(THIS%ERAW(IKPT,ISPIN)%V)) THEN
                ALLOCATE(THIS%ERAW(IKPT,ISPIN)%V(NB2-NOCC))
              END IF
              IF(.NOT.ALLOCATED(THIS%SRAW(IKPT,ISPIN)%V)) THEN
                ALLOCATE(THIS%SRAW(IKPT,ISPIN)%V(NB2-NOCC))
              END IF
            END IF
            CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%ERAW(IKPT,ISPIN)%V)
            CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%SRAW(IKPT,ISPIN)%V)
            ! DIVIDE BY BASIS WEIGHT
            IF(TBASISWGHT) THEN
              THIS%SRAW(IKPT,ISPIN)%V=THIS%SRAW(IKPT,ISPIN)%V/ADETPROD(IKPT,1)
            END IF
            ! NORMALIZE RAW SPECTRUM
            IF(TNORMALIZE) THEN
              THIS%SRAW(IKPT,ISPIN)%V=THIS%SRAW(IKPT,ISPIN)%V/ADETSUM
            END IF
          ENDDO
        ENDDO
        CALL XAS$UNSELECT
...
!     **************************************************************************
      ...
      USE STRINGS_MODULE
      IMPLICIT NONE
      ! RECL IS MAUNALLY SET IN FILEHANDLER_OPEN AS 1000
      ! PROTECTS FROM OVERFLOW AND CUTS NUMBER OF ENTRIES IF NECESSARY
...
!     **************************************************************************
      ...
      DO ISPEC=1,NSPEC
        ...
        ! WRITE RAW FILE
        IF(TRAW) THEN
          CALL FILEHANDLER$SETFILE(ID,.FALSE.,TRIM(ADJUSTL(THIS%FILE))//-'raw')
          CALL FILEHANDLER$UNIT(ID,NFIL)
          CALL XAS_OUTPUTHEADER(NFIL)
          DO IKPT=1,NKPT
            DO ISPIN=1,NSPIN
     ...
     &            I+NOCC
        ...
        IF(TKPTSPIN) THEN
          ...
          CALL FILEHANDLER$UNIT(ID,NFIL)
          CALL XAS_OUTPUTHEADER(NFIL) 
          ! LENGTH = (NKPT*NSPIN+1)*15  (BECAUSE OF ES14.5,1X)
          IF((NKPT*NSPIN+1)*15.GT.RECL) THEN
            ! (RECL/15-1)/NSPIN = NKPTMAX
            NKPTMAX=FLOOR((REAL(RECL,KIND=8)/15.D0-1.D0)/REAL(NSPIN,KIND=8))
            WRITE(NFIL,FMT='(A)')'# WARNING: NKPTMAX EXCEEDED, TRUNCATING OUTPUT'
          ELSE
            NKPTMAX=NKPT
          END IF
...
!     **************************************************************************
      ...
      DO ISPEC=1,NSPEC
        ...
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            IF(ALLOCATED(THIS%ERAW(IKPT,ISPIN)%V)) THEN 
              DEALLOCATE(THIS%ERAW(IKPT,ISPIN)%V)
            END IF
            IF(ALLOCATED(THIS%SRAW(IKPT,ISPIN)%V)) THEN 
              DEALLOCATE(THIS%SRAW(IKPT,ISPIN)%V)
            END IF
          END DO ! END ISPIN
        ENDDO ! END IKPT
        IF(ALLOCATED(THIS%ERAW)) DEALLOCATE(THIS%ERAW)
        IF(ALLOCATED(THIS%SRAW)) DEALLOCATE(THIS%SRAW)
        CALL XAS$UNSELECT
      ENDDO ! END ISPEC
...
!     **************************************************************************
      ...
      WRITE(NFIL,'(80("#"))')
      IF(TMOMENTUM) THEN
        WRITE(NFIL,FMT='(A)')'MOMENTUM TRANSFER USED (MOMENTUMTRANSFER=T)'
      ELSE
        WRITE(NFIL,FMT='(A)')'NO MOMENTUM TRANSFER USED (MOMENTUMTRANSFER=F)'
      END IF
      IF(TADET) THEN
        WRITE(NFIL,FMT='(A)')'ADET USED IN CALCULATION (ADET=T)'
      ELSE
...
!     **************************************************************************
      ...
      DO IKPTTOT=1,NKPTTOT
        ...
        DO ISPIN=1,NSPIN
          CALL KSMAP$WORKTASK(IKPT,ISPIN,WTASK)
          IF(THISTASK.NE.WTASK) CYCLE
                          CALL TRACE$I4VAL(' TWOKPT IKPTTOT',IKPTTOT)
                          CALL TRACE$I4VAL(' TWOKPT ISPIN',ISPIN)
          ! GET NB1,NB2,NOCC
          CALL STATE$SELECT('GROUND')
          CALL STATE$GETI4('NB',IKPT,ISPIN,NB1)
          CALL STATE$GETI4('NOCC',IKPT,ISPIN,NOCC)
...
!     **************************************************************************
      ...
      CALL SIMULATION$GETI4A('NKDIV',3,NKDIV)
      CALL SIMULATION$GETR8A('RBAS',9,RBAS)
      CALL SIMULATION$GETR8A('XKTOT',3*NKPTTOT,XKTOT)
      ...
      WRITE(NFIL)FLAG,EMIN,EMAX,THIS%ELIGHT,THIS%GAMMA,THIS%NORMAL, &
...
!          TWO CHARACTERS CORRESPOND TO THE ELEMENT SYMBOL E.G. MN OR O_
          ...
          DO IAT=1,NAT
            ...
            ENDIF
          ENDDO
          ! HANDLE CASES WHERE ONE CHARACTER SYMBOLS ARE USED WITHOUT A 
          ! FOLLOWING _ BUT A NUMBER INSTEAD LIKE 'O36' FOR OXYGEN
          IF(SYMBOL(2:2).GE.'0'.AND.SYMBOL(2:2).LE.'9') THEN
            SYMBOL=SYMBOL(1:1)//'_'
          END IF
! WARNING: HANDLING OF SPECIES THAT ARE NOT USED FOR ANY ELEMENT RESULTS IN
!          IN SYMBOL='  ', WHICH IS FURTHER TREATED IN XSETUP$NEW
          ...
          DO IB=1,NB
            ...
            DO LN=1,LNX(ISP)
              ...
              DO M=-L,L
                ...
                ! X COMPONENT
                CALL SPHERICAL$GAUNT(LLVAL,4,LLCORE,GAUNT(1))
                ! Y COMPONENT
                CALL SPHERICAL$GAUNT(LLVAL,2,LLCORE,GAUNT(2))
                ! Z COMPONENT
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XRAY_SORBITAL_TEST  ! MARK: XRAY_SORBITAL_TEST
!     **************************************************************************
!     ** TEST OVERLAP BETWEEN S-ORBITALS FROM THE TWO CALCULATIONS            **
!     **************************************************************************
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      LOGICAL(4) :: TOVERLAP
      INTEGER(4) :: IATOM_G,IATOM_E
      INTEGER(4) :: ISP_G,ISP_E
      INTEGER(4) :: NSP_G,NSP_E
      INTEGER(4) :: GID_G,GID_E
      INTEGER(4) :: LNXX_G,LNXX_E
      INTEGER(4), ALLOCATABLE :: LOX_G(:,:) ! (LNXX_G,ISP_G)
      INTEGER(4), ALLOCATABLE :: LOX_E(:,:) ! (LNXX_E,ISP_E)
      INTEGER(4) :: NAT
      INTEGER(4) :: NBATOM_G,NBATOM_E
      INTEGER(4), ALLOCATABLE :: ATOMMAP(:)  ! (NAT)
      INTEGER(4), ALLOCATABLE :: ISPECIES_G(:)  ! (NAT)
      INTEGER(4), ALLOCATABLE :: ISPECIES_E(:)  ! (NAT)
      INTEGER(4), ALLOCATABLE :: LNX_G(:)  ! (NSP)
      INTEGER(4), ALLOCATABLE :: LNX_E(:)  ! (NSP)
      INTEGER(4) :: GIDG,GIDE
      INTEGER(4) :: NR_G,NR_E
      REAL(8), ALLOCATABLE :: AEPSI_G(:,:) ! (NR,NBATOM)
      REAL(8), ALLOCATABLE :: AEPHI_G(:,:) ! (NR,LNX(ISP))
      REAL(8), ALLOCATABLE :: AEPSI_E(:,:) ! (NR,NBATOM)
      REAL(8), ALLOCATABLE :: AEPHI_E(:,:) ! (NR,LNX(ISP))
      REAL(8), ALLOCATABLE :: R(:)          ! (NR)
      REAL(8), ALLOCATABLE :: WORK(:)
...
!     **************************************************************************
      ...
      CALL KSMAP$READTASK(RTASK)
      IF(THISTASK.NE.RTASK) RETURN
                          CALL TRACE$PUSH('XRAY_SORBITAL_TEST')
      CALL FILEHANDLER$UNIT('PROT',NFIL)
      CALL XCNTL$GETL4('TOVERLAP',TOVERLAP)
      IF(TOVERLAP) THEN
        WRITE(NFIL,FMT='(A)')'START FROM OVERLAP FILE - CORE OVERLAP NOT AVAILABLE'
        WRITE(NFIL,FMT='(80("-"))')
        RETURN
      END IF
      ! GROUND STATE
      ...
      CALL SIMULATION$GETI4('NAT',NAT)
      CALL SIMULATION$GETI4('NSP',NSP_G)
      CALL SIMULATION$GETI4('LNXX',LNXX_G)
      ALLOCATE(ATOMMAP(NAT))
      ALLOCATE(ISPECIES_G(NAT))
      ALLOCATE(LNX_G(NSP_G))
      ALLOCATE(LOX_G(LNXX_G,NSP_G))
      CALL SIMULATION$GETI4A('ATOMMAP',NAT,ATOMMAP)
      CALL SIMULATION$GETI4A('ISPECIES',NAT,ISPECIES_G)
      CALL SIMULATION$GETI4A('LNX',NSP_G,LNX_G)
      CALL SIMULATION$GETI4A('LOX',LNXX_G*NSP_G,LOX_G)
      CALL SETTINGS$GETI4('IATOM',IATOM_G)
      ISP_G=ISPECIES_G(IATOM_G)

      CALL XSETUP$GETI4('NBATOM',ISP_G,NBATOM_G)
      CALL XSETUP$GETI4('GID',ISP_G,GID_G)
      CALL RADIAL$GETI4(GID_G,'NR',NR_G)

      ALLOCATE(AEPSI_G(NR_G,NBATOM_G))
      ALLOCATE(AEPHI_G(NR_G,LNX_G(ISP_G)))
      ALLOCATE(R(NR_G))

      CALL XSETUP$GETR8A('AEPSI',ISP_G,NR_G*NBATOM_G,AEPSI_G)
      CALL XSETUP$GETR8A('AEPHI',ISP_G,NR_G*LNX_G(ISP_G),AEPHI_G)
      CALL RADIAL$R(GID_G,NR_G,R)
      CALL XSETUP$UNSELECT
      CALL SIMULATION$UNSELECT
      ...
      CALL SIMULATION$GETI4('NAT',NAT)
      CALL SIMULATION$GETI4('NSP',NSP_E)
      CALL SIMULATION$GETI4('LNXX',LNXX_E)
      ALLOCATE(ISPECIES_E(NAT))
      ALLOCATE(LNX_E(NSP_E))
      ALLOCATE(LOX_E(LNXX_E,NSP_E))
      CALL SIMULATION$GETI4A('ISPECIES',NAT,ISPECIES_E)
      CALL SIMULATION$GETI4A('LNX',NSP_E,LNX_E)
      CALL SIMULATION$GETI4A('LOX',LNXX_E*NSP_E,LOX_E)
      IATOM_E=ATOMMAP(IATOM_G)
      ISP_E=ISPECIES_E(IATOM_E)

      CALL XSETUP$GETI4('NBATOM',ISP_E,NBATOM_E)
      CALL XSETUP$GETI4('GID',ISP_E,GID_E)
      CALL RADIAL$GETI4(GID_E,'NR',NR_E)

      ALLOCATE(AEPSI_E(NR_E,NBATOM_E))
      ALLOCATE(AEPHI_E(NR_E,LNX_E(ISP_E)))

      CALL XSETUP$GETR8A('AEPSI',ISP_E,NR_E*NBATOM_E,AEPSI_E)
      CALL XSETUP$GETR8A('AEPHI',ISP_E,NR_E*LNX_E(ISP_E),AEPHI_E)
      CALL RADIAL$R(GID_E,NR_E,R)
      CALL XSETUP$UNSELECT
      CALL SIMULATION$UNSELECT

      IF(NR_E.NE.NR_G) THEN
        CALL ERROR$MSG('NUMBER OF RADIAL POINTS NOT THE SAME')
        CALL ERROR$MSG('RADIAL$CHANGEGRID NOT IMPLEMENTED YET')
        CALL ERROR$I4VAL('NR_G',NR_G)
        CALL ERROR$I4VAL('NR_E',NR_E)
        CALL ERROR$MSG('XRAY_SORBITAL_TEST')
        WRITE(NFIL,FMT='(A)')'NUMBER OF RADIAL POINTS NOT THE SAME'
        WRITE(NFIL,FMT='(A,I4,A,I4)')'NR_G = ',NR_G,'NR_E = ',NR_E
        WRITE(NFIL,FMT='(A)')'XRAY_SORBITAL_TEST NOT IMPLEMENTED YET FOR DIFFERENT GRIDS'
        WRITE(NFIL,FMT='(80("-"))')
        RETURN
      END IF
      ...
      ALLOCATE(WORK(NR_G))
      WRITE(NFIL,FMT='(2A14)')'AEPSI','AEPSI'
      ! G 1S 1S G
      WORK=AEPSI_G(:,1)*R**2*AEPSI_G(:,1)
      CALL RADIAL$INTEGRAL(GID_G,NR_G,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <1S|','|1S> GS: ',SVAR
      ! G 1S 2S G
      WORK=AEPSI_G(:,1)*R**2*AEPSI_G(:,2)
      CALL RADIAL$INTEGRAL(GID_G,NR_G,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <1S|','|2S> GS: ',SVAR
      ! E 1S 1S E
      WORK=AEPSI_E(:,1)*R**2*AEPSI_E(:,1)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'EX <1S|','|1S> EX: ',SVAR
      ! E 1S 2S E
      WORK=AEPSI_E(:,1)*R**2*AEPSI_E(:,2)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'EX <1S|','|2S> EX: ',SVAR
      ! G 1S 1S E
      WORK=AEPSI_G(:,1)*R**2*AEPSI_E(:,1)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <1S|','|1S> EX: ',SVAR
      ! G 1S 2S E
      WORK=AEPSI_G(:,1)*R**2*AEPSI_E(:,2)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <1S|','|2S> EX: ',SVAR
      ! G 2S 1S E
      WORK=AEPSI_G(:,2)*R**2*AEPSI_E(:,1)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <2S|','|1S> EX: ',SVAR
      ! G 2S 2S E
      WORK=AEPSI_G(:,2)*R**2*AEPSI_E(:,2)
      CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
      WRITE(NFIL,FMT='(2A14,F14.8)')'GS <2S|','|2S> EX: ',SVAR

      WRITE(NFIL,FMT='(80("-"))')
      WRITE(NFIL,FMT='(2A14)')'AEPSI','PART. WAVES'
      DO LN=1,LNX_G(ISP_G)
        L=LOX_G(LN,ISP_G)
        IF(L.NE.0) CYCLE
        WORK=AEPHI_G(:,LN)*R**2*AEPSI_G(:,1)
        CALL RADIAL$INTEGRAL(GID_G,NR_G,WORK,SVAR)
        WRITE(NFIL,FMT='(A14,A7,I1,A6,F14.8)')'GS <1S|','|S_',LN,'> GS: ',SVAR
      END DO

      DO LN=1,LNX_E(ISP_E)
        L=LOX_E(LN,ISP_E)
        IF(L.NE.0) CYCLE
        WORK=AEPHI_E(:,LN)*R**2*AEPSI_E(:,1)
        CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
        WRITE(NFIL,FMT='(A14,A7,I1,A6,F14.8)')'EX <1S|','|S_',LN,'> EX: ',SVAR
      END DO
      
      DO LN=1,LNX_E(ISP_E)
        L=LOX_E(LN,ISP_E)
        IF(L.NE.0) CYCLE
        WORK=AEPHI_E(:,LN)*R**2*AEPSI_G(:,1)
        CALL RADIAL$INTEGRAL(GID_E,NR_E,WORK,SVAR)
        WRITE(NFIL,FMT='(A14,A7,I1,A6,F14.8)')'GS <1S|','|S_',LN,'> EX: ',SVAR
      END DO

      DO LN=1,LNX_G(ISP_G)
        L=LOX_G(LN,ISP_G)
        IF(L.NE.0) CYCLE
        WORK=AEPHI_G(:,LN)*R**2*AEPSI_E(:,1)
        CALL RADIAL$INTEGRAL(GID_G,NR_G,WORK,SVAR)
        WRITE(NFIL,FMT='(A14,A7,I1,A6,F14.8)')'EX <1S|','|S_',LN,'> GS: ',SVAR
      END DO

      WRITE(NFIL,FMT='(80("-"))')

      DEALLOCATE(ATOMMAP)
      DEALLOCATE(ISPECIES_E)
      DEALLOCATE(LNX_E)
      DEALLOCATE(AEPSI_E)
      DEALLOCATE(AEPHI_E)
      DEALLOCATE(R)
      DEALLOCATE(ISPECIES_G)
      DEALLOCATE(LNX_G)
      DEALLOCATE(AEPSI_G)
      DEALLOCATE(AEPHI_G)
                          CALL TRACE$POP

      END SUBROUTINE XRAY_SORBITAL_TEST
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
...
!     **************************************************************************
      ...
      CALL SIMULATION$UNSELECT
      CALL XCNTL$GETL4('ADET',TADET)
      CALL XAS$GETL4('SINGLEPARTICLE',TSINGLEPARTICLE)
      DO IORB=1,NAUXORB
        CALL AUXORB$SELECT(IORB)
        IKPT=THIS%K
        ISPIN=THIS%S
        ...
        ALLOCATE(THIS%AMPL(3,NOCC+1))
        ALLOCATE(THIS%DIPOLE(3,NOCC+1))
        DO IB=1,NOCC
          THIS%VAL(IB)=-KMAT(IEMP,IB)
          THIS%BARR(IB)=IB
          THIS%AMPL(:,IB)=-CONJG(KMAT(IEMP,IB))*DIPOLE(:,IB)
          THIS%DIPOLE(:,IB)=DIPOLE(:,IB)
        ENDDO
        IF(TSINGLEPARTICLE) THEN
          THIS%VAL(:)=0.D0
          THIS%AMPL(:,:)=0.D0
        END IF
        THIS%VAL(NOCC+1)=1.D0
        THIS%AMPL(:,NOCC+1)=DIPOLE(:,IEMP)
        THIS%BARR(NOCC+1)=BAND
        ! TODO: CHECK IF THIS IS NEEDED
        THIS%VAL(:)=THIS%VAL(:)*(-1.D0)**(NOCC)
        THIS%AMPL(:,:)=THIS%AMPL(:,:)*(-1.D0)**(NOCC)
        IF(TADET) THEN 
          THIS%VAL(:)=THIS%VAL(:)*ADET*ADETOP
          THIS%AMPL(:,:)=THIS%AMPL(:,:)*CONJG(ADET)*CONJG(ADETOP)
        END IF
        DEALLOCATE(KMAT)
        DEALLOCATE(DIPOLE)
        CALL OVERLAP$UNSELECT
...
!     **************************************************************************
      ...
      DO IORB=1,NAUXORB
        ...
        IF(THISTASK.EQ.RTASK) THEN
          ...
          IF(.NOT.ALLOCATED(THIS%AMPL)) ALLOCATE(THIS%AMPL(3,THIS%NOCC+1))
          IF(.NOT.ALLOCATED(THIS%DIPOLE)) ALLOCATE(THIS%DIPOLE(3,THIS%NOCC+1))
        END IF
        CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%VAL)
        CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%BARR)
        CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%AMPL)
        CALL MPE$SENDRECEIVE('~',WTASK,RTASK,THIS%DIPOLE)
        IF(THISTASK.EQ.RTASK) THEN
          WRITE(NFIL,FMT='("# AUXILIARY ORBITALS K=",I5," S=",I2," BAND=",I4)')&
     &               IKPT,ISPIN,BAND
          WRITE(NFIL,FMT='(A5, 8A14)') &
     &               '#BAND', 'REAL', 'IMAG','|AMPL_X|','|AMPL_Y|','|AMPL_Z|', &
     &               '|DIPOLE_X|','|DIPOLE_Y|','|DIPOLE_Z|'
          DO IB=1,THIS%NOCC+1
            WRITE(NFIL,FMT='(I5,8F14.8)') THIS%BARR(IB), &
     &       REAL(THIS%VAL(IB)),AIMAG(THIS%VAL(IB)), &
     &       ABS(THIS%AMPL(1,IB)),ABS(THIS%AMPL(2,IB)),ABS(THIS%AMPL(3,IB)), &
     &       ABS(THIS%DIPOLE(1,IB)),ABS(THIS%DIPOLE(2,IB)),ABS(THIS%DIPOLE(3,IB))
          ENDDO
        END IF
        IF(THISTASK.EQ.RTASK) CALL FILEHANDLER$CLOSE('AUXORB')
        DEALLOCATE(THIS%VAL)
        DEALLOCATE(THIS%BARR)
        DEALLOCATE(THIS%AMPL)
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$CALCULATE  ! MARK: XASDOS$CALCULATE
!     **************************************************************************
...
!          IN PAW_DOS. THIS IS TO MATCH THE SHAPE OF THE XAS SPECTRUM.
      DO ISPEC=1,NSPEC
        ...
        CALL XAS$GETCH('BROADMODE',BROADMODE)
        DO ISET=1,NSET
          DO ISPIN=1,NSPIN
            IF(BROADMODE.EQ.'G') THEN
              CALL GAUSSCONV(NE,EDOS,DOS(:,ISPIN,ISET,ISPEC),EBROAD)
            ELSE IF(BROADMODE.EQ.'L') THEN
              CALL LORENTZCONV(NE,EDOS,DOS(:,ISPIN,ISET,ISPEC),EBROAD)
            ELSE IF(BROADMODE.EQ.'N') THEN
              ! NO BROADENING
            ELSE
              CALL ERROR$MSG('BROADMODE NOT RECOGNISED IN XASDOS MODULE')
              CALL ERROR$CHVAL('BROADMODE',BROADMODE)
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ...
          IF(THISTASK.EQ.RTASK) THEN
            ...
            ! AUXILIARY ORBITALS ARE BEING NORMALIZED BY SQRT(<AUX_F|AUX_F>)
            ! TO CONFORM WITH THE CALCULATION OF PROJECTED DOS
! WARNING: CAREFUL WITH USING THE AUXILIARY ORBITALS FOR FURTHER CALCULATIONS
            ...
            DO IEMP=1,NB-NOCC
              ...
              DO IB=1,NOCC
                ...
                NORM=NORM+ABS(KMAT(IEMP,IB))**2
              ENDDO
              NORM=NORM*ABS(ADET*ADETOP)**2
              NORM=SQRT(NORM)
              ! NORMALIZE AUXILIARY ORBITAL PROJECTIONS
              PROJREORDERED(:,:,IEMPTOT)=PROJREORDERED(:,:,IEMPTOT)/NORM
            ENDDO
            ! MULTIPLY BY (-1)^NOCC
...
!     **************************************************************************
!     ** READ DOS RAW DATA FROM FILE                                          **
!     **************************************************************************
      USE STRINGS_MODULE
      USE XASDOS_MODULE, ONLY: TACTIVE,FILE,EF,NBB,NKPT,NSET,EIGVAL,SETVAL,SETID
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      INTEGER(4) :: ISET
      CHARACTER(8) :: KEY
...
!     **************************************************************************
      ...
      IF(.NOT.KEY.EQ.+'FINISHED') THEN
        ...
        CALL ERROR$CHVAL('KEY',KEY)
        CALL ERROR$STOP('XASDOS$READ')
      END IF
      CALL FILEHANDLER$CLOSE('XASDOS')
                          CALL TRACE$POP
      RETURN
      END SUBROUTINE XASDOS$READ
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$VALIDATE  ! MARK: XASDOS$VALIDATE
!     **************************************************************************
!     ** VALIDATE XASDOS MODULE DATA                                          **
!     ** DATA SHOULD CORRESPOND TO DATA CONTAINED IN THE EXCITATED SIMULATION **
!     **************************************************************************
      USE XASDOS_MODULE, ONLY: TACTIVE,NBB,NKPT,EIGVAL,EIG,SETVAL,SET,NSET,SETID
      IMPLICIT NONE
      INTEGER(4) :: NKPTSIM
      INTEGER(4) :: NSPINSIM
      INTEGER(4) :: NBSIM
...
!     **************************************************************************
     ...
     &                  SIMULATIONS')
      ...
      CALL SIMULATION$SELECT('GROUND')
      CALL SIMULATION$GETR8('ETOT',ETOTGROUND)
      CALL SIMULATION$UNSELECT
     ...
     &                        EXCITATED SIMULATION')
              ...
              CALL ERROR$R8VAL('XASDOS EIG',EIGVAL(IB+NBSIM*(ISPIN-1),IKPT))
              CALL ERROR$R8VAL('EXCITATED EIG',EIGSIM(IB))
              CALL ERROR$STOP('XASDOS$VALIDATE')
            END IF
          ENDDO
          DEALLOCATE(EIGSIM)
      ...
      DO ISPIN=1,NSPINSIM
        DO IKPT=1,NKPT
          DO IB=1,NB
            EIG(IB,IKPT,ISPIN)=EIGVAL(IB+NB*(ISPIN-1),IKPT)
          ENDDO
        ENDDO
      ENDDO
...
!     **************************************************************************
      USE STRINGS_MODULE
      USE XASDOS_MODULE, ONLY: TACTIVE,NSET,SETID,FILE,EMIN,EMAX,DE
      IMPLICIT NONE
      INTEGER(4), INTENT(IN) :: NFIL
      LOGICAL(4) :: TOVERLAP
      INTEGER(4) :: ISPEC
...
!     **************************************************************************
     ...
     &                         NBB,EIG,EIGVAL
      ...
      INTEGER(4) :: I
      REAL(8) :: EV
      TYPE(EWGHT_TYPE), ALLOCATABLE :: EWGHTVAL(:,:) ! (NBB,NKPT)
      REAL(8), ALLOCATABLE :: WGHTVAL(:,:) ! (NBB,NKPT)
      INTEGER(4) :: RTASK,THISTASK,NTASKS
!     **************************************************************************
      IF(.NOT.TACTIVE) RETURN
      CALL MPE$QUERY('~',NTASKS,THISTASK)
      CALL KSMAP$READTASK(RTASK)
      ...
      CALL SIMULATION$SELECT('EXCITE')
      CALL SIMULATION$GETI4('NSPIN',NSPIN)
      CALL SIMULATION$GETR8('RNTOT',RNTOT)
      CALL SIMULATION$UNSELECT
      
      ! ALLOCATE ENERGY GRID AND DOS ARRAYS
      ALLOCATE(EDOS(NE))
      ALLOCATE(DOS(NE,NSPIN,NSET,NSPEC))
...
! TODO: CHECK IF MAPPING IS CORRECT
      ...
      DO ISPIN=1,NSPIN
        DO IKPT=1,NKPT
          DO I=1,NB
            WGHT(I,IKPT,ISPIN)=WGHTVAL(I+NB*(ISPIN-1),IKPT)
            EWGHT(I,IKPT,ISPIN)=EWGHTVAL(I+NB*(ISPIN-1),IKPT)
          ENDDO
        ENDDO
      ENDDO
      DEALLOCATE(WGHTVAL)
      DEALLOCATE(EWGHTVAL)
                          CALL TRACE$POP
      RETURN
      END SUBROUTINE XASDOS_INITIALIZE
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$SETL4(ID,VAL)  ! MARK: XASDOS$SETL4
!     **************************************************************************
!     ** SET LOGICAL VALUE IN XASDOS MODULE                                   **
!     **************************************************************************
      USE XASDOS_MODULE, ONLY: TACTIVE
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      LOGICAL(4), INTENT(IN) :: VAL
!     **************************************************************************
      ...
      ELSE
        CALL ERROR$MSG('ID NOT RECOGNISED')
        CALL ERROR$CHVAL('ID',ID)
        CALL ERROR$STOP('XASDOS$SETL4')
      END IF
      RETURN
      END SUBROUTINE XASDOS$SETL4
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$GETI4(ID,VAL)  ! MARK: XASDOS$GETI4
!     **************************************************************************
...
!     **************************************************************************
      ...
      END IF
      RETURN
      END SUBROUTINE XASDOS$GETI4
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$GETCH(ID,VAL)  ! MARK: XASDOS$GETCH
!     **************************************************************************
!     ** GET CHARACTER VALUE FROM XASDOS MODULE                               **
!     **************************************************************************
      USE XASDOS_MODULE, ONLY: TACTIVE,FILE
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      CHARACTER(*), INTENT(OUT) :: VAL
!     **************************************************************************
      ...
      END IF
      RETURN
      END SUBROUTINE XASDOS$GETCH
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASDOS$SETCH(ID,VAL)  ! MARK: XASDOS$SETCH
!     **************************************************************************
!     ** SET CHARACTER VALUE IN XASDOS MODULE                                 **
!     **************************************************************************
      USE XASDOS_MODULE, ONLY: FILE
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      CHARACTER(*), INTENT(IN) :: VAL
!     **************************************************************************
