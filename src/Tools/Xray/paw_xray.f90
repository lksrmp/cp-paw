...
!     **************************************************************************
      ...
      LOGICAL(4), ALLOCATABLE :: TINVARR(:) ! (NKPT) INVERSION SYMMETRY FOR EACH K-POINT
...
! TODO: CHECK FUNCTION FOR ISHIFT
      ...
      DO ISPEC=1,NSPEC
        CALL RIXS$ISELECT(ISPEC)
        ! TODO: CHECK SIGN
        ! GO FROM FIRST K POINT WITH Q TRANSFER
        XQ1(:)=XK(:,1)+THIS%XQ(:)
        ! REDUCE TO FIRST BRILLOUIN ZONE
        ! XQ1(:)=MODULO(XQ1(:),1.D0)
        ...
        DISTANCE=HUGE(1.D0)
        ! LOOP THROUGH NEIGHBOURING K POINTS AND FIND BEST APPROXIMATION FOR Q
        DO DI=-1,1
          DO DJ=-1,1
            DO DK=-1,1
...
!     **************************************************************************
      ...
      IF(TXAS) THEN
        IF(ALLOCATED(ABSORB)) THEN
          CALL TRACE$PASS('ABSORB ALREADY CALCULATED IN XAS')
          CALL TRACE$POP
          RETURN
        ELSE
          CALL ERROR$MSG('ABSORB NOT CALCULATED IN XAS DESPITE XAS ACTIVE')
          CALL ERROR$STOP('RIXS_ABSORB')
        END IF
      END IF
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ...
          ALLOCATE(ABSORB(IKPT,ISPIN)%AMPL(3,NB2-NOCC))
          DO IEMP=1,NB2-NOCC
            CALL AMPLITUDE(NB2,NOCC,IEMP,KMAT,DIPOLE,ABSORB(IKPT,ISPIN)%AMPL(:,IEMP))
          ENDDO ! END IEMP
          DEALLOCATE(KMAT)
          DEALLOCATE(DIPOLE)
        ENDDO ! END ISPIN
...
!     **************************************************************************
      ...
      DO IKPT=1,NKPT
        IINVARR(IKPT)=0
        IF(TINVARR(IKPT)) IINVARR(IKPT)=1
      ENDDO
      DEALLOCATE(TINVARR)
      ALLOCATE(R(3,NAT))
      CALL SIMULATION$GETR8A('R',3*NKPT,R)
      ...
      WRITE(NFIL)NKPT,NSPIN,NKDIV,RHOLE
      ! TINVARR(IINVARR)
      WRITE(NFIL)IINVARR
      DEALLOCATE(IINVARR)
      ...
      DO IKPT=1,NKPT
        ...
        DO ISPIN=1,NSPIN
          ...
          IF(THIS%TKPTSHIFT) THEN
            ...
            ! THIS%ONEAMPL(IKPTTOT,ISPIN)%X(NB1-NOCC)
            WRITE(NFIL)THIS%ONEAMPL(IKPTTOT,ISPIN)%X
            ! THIS%ONEAMPL(IKPTTOT,ISPIN)%Y(NOCC)
            WRITE(NFIL)THIS%ONEAMPL(IKPTTOT,ISPIN)%Y
          ELSE
            ! == OUTPUT AMPLITUDES FOR ONEKPT SPECTRUM =========================
            ! THIS%ONEAMPL(IKPTTOT,ISPIN)%XY(NB1-NOCC,NOCC)
            WRITE(NFIL)THIS%ONEAMPL(IKPTTOT,ISPIN)%XY
          END IF
          
          ! IF NEGATIVE K-POINT, OUTPUT AMPLITUDES FOR NEGATIVE K-POINT
          IF(TNEGATIVEKPT) THEN
            WRITE(NFIL)IKPTTOT+1,ISPIN,NB1,NOCC
            WRITE(NFIL)EIG
            IF(THIS%TKPTSHIFT) THEN
              ! THIS%ONEAMPL(IKPTTOT+1,ISPIN)%X(NB1-NOCC)
              WRITE(NFIL)THIS%ONEAMPL(IKPTTOT+1,ISPIN)%X
              ! THIS%ONEAMPL(IKPTTOT+1,ISPIN)%Y(NOCC)
              WRITE(NFIL)THIS%ONEAMPL(IKPTTOT+1,ISPIN)%Y
            ELSE
              ! THIS%ONEAMPL(IKPTTOT+1,ISPIN)%XY(NB1-NOCC,NOCC)
              WRITE(NFIL)THIS%ONEAMPL(IKPTTOT+1,ISPIN)%XY
            END IF
          END IF
          DEALLOCATE(EIG)
        ENDDO ! END ISPIN
