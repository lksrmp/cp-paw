          ...
          DO I=1,NOCC
            DO J=1,NOCC
              DO M=1,3
                ...
                CVAR1=CVAR1+CONJG(DIPOLE(M,I))*AINV(J,I)*DIPOLEGS(M,J)
              ENDDO
              DO K=1,NOCC
                DO M=1,3
                  ! <C|O^+ P_K^+ P_K O|C>
                  CVAR2=CVAR2+CONJG(DIPOLE(M,I))*CONJG(AINV(J,I))*AINV(J,K)*DIPOLE(M,K)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
          ! <C|O^+ (1 - P_K - P_K^+ + P_K^+ P_K) O|C>
          INFTOTALKPT=TOTALWKPT*(CVAR+CONJG(CVAR1)-CVAR2)
          IF(TNEGATIVEKPT) THEN
            ...
            INFTOTALKPT=INFTOTALKPT+TOTALWKPT*(CONJG(CVAR)+CVAR1-CONJG(CVAR2))
          END IF
          IF(TADET) THEN
