...
!     **************************************************************************
     ...
     &                                 THIS%WKPT(IKPT),THIS%TINVARR(IKPT)
      ...
      WRITE(NFIL,FMT='(A10,A10,A10)')'KPTTOT','IKPT','IRR. KPT'
      DO IKPT=1,THIS%NKPTTOT
        WRITE(NFIL,FMT='(2I10,L10)')IKPT,THIS%TOIRRKPT(IKPT),THIS%TIRRKPT(IKPT)
      ENDDO
      WRITE(NFIL,FMT='(A10,A10)')'IKPT','IKPTTOT'
      DO IKPT=1,THIS%NKPT
        WRITE(NFIL,FMT='(2I10)')IKPT,THIS%TOTOTKPT(IKPT)
      ENDDO
      FLUSH(NFIL)
                          CALL TRACE$POP
      RETURN
...
!     **************************************************************************
      ...
      DO ISPEC=1,NSPEC
        ...
        IF(THIS%TKPTSHIFT) THEN
          ...
          DO IKPT=1,NKPTTOT
            WRITE(NFIL,FMT='(2I10)') IKPT,THIS%JOFIKPT(IKPT)
          ENDDO
        END IF
        CALL RIXS$UNSELECT
      ENDDO
...
! TODO: CHECK FUNCTION FOR ISHIFT
      ...
      DO ISPEC=1,NSPEC
        ...
        THIS%XQAPPROX(:)=THIS%XQAPPROX(:)*REAL(NKDIV(:),KIND=8)
        BASE(:)=NINT(THIS%XQAPPROX(:))
        THIS%XQAPPROX(:)=REAL(BASE(:),KIND=8)/REAL(NKDIV(:),KIND=8)
...
!     **************************************************************************
      ...
      COMPLEX(8), ALLOCATABLE :: Y(:)
                          CALL TRACE$PUSH('RIXS_OUTPUTDATA')
      IF(.NOT.TINITIALIZE) THEN
        CALL ERROR$MSG('RIXS NOT INITIALIZED')
