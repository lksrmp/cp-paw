...
!     **************************************************************************
      ...
      REAL(8) :: TOTAL ! TOTAL SPECTRUM (SUM_K SUM_SPIN SPECTRUM)
      REAL(8) :: GOOG ! <G|O+ O|G> INTEGRATED FULL SPECTRUM
...
!     **************************************************************************
      ...
      DO IKPT=1,NKPT
        ...
        DO ISPIN=1,NSPIN
          ...
          DO ISPEC=1,NSPEC
            ...
            DO I=1,NB1
              DIPOLESUM=DIPOLESUM+TOTALWKPT*ABS(DOT_PRODUCT(POLXYZ,DIPOLEGS(:,I)))**2
              IF(TNEGATIVEKPT) THEN
                DIPOLESUM=DIPOLESUM+TOTALWKPT*ABS(DOT_PRODUCT(POLXYZ,CONJG(DIPOLEGS(:,I))))**2
              END IF
            ENDDO
            THIS%GOOG=THIS%GOOG+DIPOLESUM
            CALL XAS$UNSELECT
          ENDDO
      ...
      DO ISPEC=1,NSPEC
        ...
        ! COMBINE TOTAL
        CALL MPE$COMBINE('~','+',THIS%TOTAL)
        ! COMBINE GOOG
        CALL MPE$COMBINE('~','+',THIS%GOOG)
        CALL OVERLAP$GETR8A('R2CORE',2,R2CORE)
        THIS%GOOG=R2CORE(1)-THIS%GOOG
        THIS%NORM=THIS%GOOG/THIS%TOTAL
        ...
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            ...
            IF(TNORMALIZE) THEN
              ...
              THIS%SPECTRUM(IKPT,ISPIN,:)=THIS%SPECTRUM(IKPT,ISPIN,:)*THIS%NORM
            END IF  
            ...
            IF(TNORMALIZE) THEN
              ...
              THIS%SRAW(IKPT,ISPIN)%V=THIS%SRAW(IKPT,ISPIN)%V*THIS%NORM
            END IF
          ENDDO
        ENDDO
        CALL XAS$UNSELECT
...
WRITE(NFIL, *) '# TOTAL SPECTRUM ',THIS%TOTAL
WRITE(NFIL, *) '# <G|O^DAGGER O|G> ',THIS%GOOG
WRITE(NFIL, *) '# NORM ',THIS%NORM
