...
!     ...1.........2.........3.........4.........5.........6.........7.........8
      ...
      INTEGER(4) :: NSPIN
      INTEGER(4) :: NKDIV(3)
      LOGICAL(4), ALLOCATABLE :: TINVARR(:) ! (NKPT)
      CHARACTER(6) :: FLAG
      REAL(8) :: EMIN
      REAL(8) :: EMAX
      REAL(8) :: ELIGHT
...
!     **************************************************************************
      ...
      ! NKPT,NSPIN,NKDIV,RHOLE
      READ(NFIL)NKPT,NSPIN,NKDIV,RHOLE
      ...
      ALLOCATE(IINVARR(NKPT))
      ALLOCATE(TINVARR(NKPT))
      ! TINVARR(IINVARR)
      READ(NFIL)IINVARR
      DO IKPT=1,NKPT
        TINVARR(IKPT)=IINVARR(IKPT).EQ.1
      END DO
      DEALLOCATE(IINVARR)
      CALL RIXSAMPL$SETL4A('TINVARR',NKPT,TINVARR)
...
!     **************************************************************************
      USE RIXSAMPL_MODULE, ONLY: NKDIV,TFIRST
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
...
!     **************************************************************************
      USE RIXSAMPL_MODULE, ONLY: NKPT,TINVARR,TFIRST
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: ID
      INTEGER(4), INTENT(IN) :: LEN
      LOGICAL(4), INTENT(IN) :: VAL(LEN)
      IF(ID.EQ.'TINVARR') THEN
        IF(TFIRST) ALLOCATE(TINVARR(NKPT))
        IF(LEN.NE.NKPT) THEN
          CALL ERROR$MSG('TINVARR LENGTH SHOULD BE NKPT')
          CALL ERROR$I4VAL('LEN',LEN)
          CALL ERROR$I4VAL('NKPT',NKPT)
          CALL ERROR$STOP('RIXSAMPL$SETL4A')
        END IF
        IF(.NOT.TFIRST) THEN
          IF(ANY(TINVARR.NEQV.VAL)) THEN
            CALL ERROR$MSG('VALUES SHOULD NOT DIFFER')
            CALL ERROR$CHVAL('ID: ',ID)
            CALL ERROR$STOP('RIXSAMPL$SETL4A')
          END IF
...
!     **************************************************************************
      USE RIXSAMPL_MODULE, ONLY: FLAG
      USE STRINGS_MODULE
      IMPLICIT NONE

      IF(FLAG.NE.+'ONEKPT') RETURN

      ! CALCULATION OF ONEKPT
      ! CONTINUE HERE
      RETURN
