...
!       ENERGY GRID PARAMETER
      ...
      TYPE SPECTRUM_TYPE
        ...
        REAL(8), ALLOCATABLE :: ERAW(:,:,:)  ! (NKPT,NSPIN,NB2)
        REAL(8), ALLOCATABLE :: IRAW(:,:,:)  ! (NKPT,NSPIN,NB2)
...
!     INDEX 1 IS GROUNDSTATE AND 2 EXCITED STATE
      ...
      INTEGER(4), ALLOCATABLE :: KSMAP(:,:) ! (NKPT,NSPING) K AND S RESPONSIBILITY OF TASKS
...
!     **************************************************************************
      USE XASCNTL_MODULE, ONLY: LL_CNTL
      USE XAS_MODULE, ONLY: SELECTED,TRSTRT,THIS
      USE LINKEDLIST_MODULE
      IMPLICIT NONE
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XASCNTL$TFERMI  ! MARK: XASCNTL$TFERMI
!     **************************************************************************
...
!     **************************************************************************
      ...
      LOGICAL(4) :: TFERMI2
!     **************************************************************************
                          CALL TRACE$PUSH('XASCNTL$TFERMI')
      CALL XAS$SELECT('GROUNDSTATE')
      TFERMI1=THIS%TFERMI
      CALL XAS$UNSELECT
      CALL XAS$SELECT('EXCITESTATE')
      ...
      IF(TFERMI1.AND..NOT.TFERMI2) THEN
        ...
        CALL ERROR$STOP('XASCNTL$TFERMI')
      END IF
...
&                           KSMAP,NKPTG,NSPING,RTASK,OVL,OVERLAPARR
      USE MPE_MODULE
      IMPLICIT NONE
      INTEGER(4) :: ISPEC
      INTEGER(4) :: IKPT
...
!     WARNING: ENERGY RANGE EXTENDED BY 2 EV
      ...
      DO ISPEC=1,SETTINGS%NSPEC
        ...
        SPEC%I(:,:,:)=0.D0
! ERROR: ALLOCATION OF SPECTRUM%ERAW AND SPECTRUM%IRAW CAN BE WRONG IF NB IS
!        NOT THE SAME FOR ALL K POINTS
        ALLOCATE(SPEC%ERAW(NKPTG,NSPING,NB))
        SPEC%ERAW(:,:,:)=0.D0
        ALLOCATE(SPEC%IRAW(NKPTG,NSPING,NB))
        SPEC%IRAW(:,:,:)=0.D0
        SPEC%I(:,:,:)=0.D0
        DO I=1,NE
...
!       LOOP OVER SPIN
        DO ISPIN=1,NSPING
          IF(KSMAP(IKPT,ISPIN).NE.THISTASK) CYCLE
                          CALL TRACE$I4VAL(' CALCULATING IKPT',IKPT)
                          CALL TRACE$I4VAL(' CALCULATING ISPIN',ISPIN)
          S=>SIM(2)%STATEARR(IKPT,ISPIN)
          OVL=>OVERLAPARR(IKPT,ISPIN)
          DO IFINAL=1,S%NB-S%NOCC
...
!             ABSOLUTE SQUARE AND WEIGHT OF K POINT
              ...
              SPEC%ERAW(IKPT,ISPIN,IFINAL+S%NOCC)=E
              SPEC%IRAW(IKPT,ISPIN,IFINAL+S%NOCC)=SIGMA
...
!     **************************************************************************
      USE XAS_MODULE, ONLY: OVL,OVERLAPARR,SIM,STATE_TYPE,NKPTG,NSPING,KSMAP
      USE MPE_MODULE
      IMPLICIT NONE
      TYPE(STATE_TYPE), POINTER :: S1,S2
      INTEGER(4) :: IKPT,ISPIN
...
&                           OVERLAPARR,TRSTRT,ATOMMAP,KSMAP,NKPTG,NSPING
      IMPLICIT NONE
...
!     **************************************************************************
      USE XAS_MODULE, ONLY: OVL,OVERLAPARR,SIM,KSMAP,NKPTG,NSPING
      IMPLICIT NONE
      INTEGER(4) :: IKPT
      INTEGER(4) :: ISPIN
      INTEGER(4) :: NTASKS,THISTASK
          ...
          DO IKPT=1,NKPTG
            DO ISPIN=1,NSPING
              ...
              DO IFINAL=1,NB2-NOCC
                I=IFINAL+NOCC
                WRITE(NFIL,*)SPEC%ERAW(IKPT,ISPIN,I)/EV, &
&                            SPEC%IRAW(IKPT,ISPIN,I),I
              ENDDO
            ENDDO
          ENDDO
...
!     **************************************************************************
      USE MPE_MODULE
      USE XAS_MODULE, ONLY: OUTPUT,SIM,NKPTG,NSPING,NSIM,KSMAP,RTASK,OVL,OVERLAPARR,STATE_TYPE
      IMPLICIT NONE
      INTEGER(4) :: IKPT
      INTEGER(4) :: ISPIN
      INTEGER(4) :: IS
...
!         OVERLAP PW AND AUG MANDATORY FOR RESTART FILE
          ...
          IF(OUTPUT%TOVL) THEN
            IF(.NOT.ALLOCATED(OVL%OV)) THEN
              ALLOCATE(OVL%OV(S2%NB,S1%NB))
            END IF
            CALL MPE$SENDRECEIVE('~',KSMAP(IKPT,ISPIN),RTASK,OVL%OV)
          END IF
          IF(OUTPUT%TOVLOCC) THEN
            IF(.NOT.ALLOCATED(OVL%OVOCC)) THEN
              ALLOCATE(OVL%OVOCC(S2%NOCC,S1%NOCC))
            END IF
            CALL MPE$SENDRECEIVE('~',KSMAP(IKPT,ISPIN),RTASK,OVL%OVOCC)
          END IF
          IF(OUTPUT%TOVLEMP) THEN
            IF(.NOT.ALLOCATED(OVL%OVEMP)) THEN
              ALLOCATE(OVL%OVEMP(S2%NB-S2%NOCC,S1%NOCC))
            END IF
            CALL MPE$SENDRECEIVE('~',KSMAP(IKPT,ISPIN),RTASK,OVL%OVEMP)
          END IF
          IF(OUTPUT%TKMAT) THEN
            IF(.NOT.ALLOCATED(OVL%KMAT)) THEN
              ALLOCATE(OVL%KMAT(S2%NB-S2%NOCC,S1%NOCC))
            END IF
            CALL MPE$SENDRECEIVE('~',KSMAP(IKPT,ISPIN),RTASK,OVL%KMAT)
          END IF
          IF(OUTPUT%TDIPOLE) THEN
...
!     **************************************************************************
      USE XAS_MODULE, ONLY: THIS,OVL,OVERLAPARR,STATE_TYPE
      IMPLICIT NONE
      INTEGER(4) :: NFIL
...
!     **************************************************************************
      USE XAS_MODULE, ONLY: THIS,SIM,OVL,OVERLAPARR,KSMAP,NKPTG,NSPING,RTASK,STATE_TYPE
      USE MPE_MODULE
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      TYPE(STATE_TYPE), POINTER :: S
...
!       BOTH READ TASK AND RESPONSIBLE FOR EVERY KPOINT
      IF(NTASKS.EQ.1) THEN
        ...
        KSMAP=1
!     MORE THAN ONE TASK:
!       FIRST IS READ TASK, OTHERS ARE RESPONSIBLE FOR DIFFERENT KPOINTS / SPINS
      ELSE
        RTASK=1
        DO IKPT=1,NKPTG
          DO ISPIN=1,NSPING
