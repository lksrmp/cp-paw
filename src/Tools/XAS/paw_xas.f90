! WARNING: CHECK FOR HANDLING OF SMALL NUMBERS DURING CALCULATION
! ERROR: HANDLING OF SIGN OF WAVEFUNCTIONS
!........1.........2.........3.........4.........5.........6.........7.........8
      MODULE XAS_MODULE
      TYPE SETUP_TYPE
...
!     **************************************************************************
      USE XAS_MODULE, ONLY: SETTINGS,SPECTRUM,SPECTRUMARR,OUTPUT
      USE STRINGS_MODULE
      IMPLICIT NONE
      INTEGER(4) :: NFIL
      INTEGER(4) :: ISPEC
...
! WARNING: CHECK INDICES
      DO IB=1,STATE%NOCC
        AMPL(:)=AMPL(:)+CONJG(OVERLAP%KMAT(IFINAL,IB))*OVERLAP%DIPOLE(:,IB)
      ENDDO
...
!        NOCC ARE NOT THE SAME FOR ALL K POINTS
        ALLOCATE(SPECTRUM%ERAW(SIM(2)%NKPT,SIM(2)%NSPIN,STATE%NB-STATE%NOCC))
        ALLOCATE(SPECTRUM%IRAW(SIM(2)%NKPT,SIM(2)%NSPIN,STATE%NB-STATE%NOCC))
        SPECTRUM%I(:,:,:)=0.D0
        DO I=1,NE
...
!           LOOP OVER SPECTRA
            DO ISPEC=1,SETTINGS%NSPEC
              ...
              SIGMA=REAL(CONJG(CVAR)*CVAR,KIND=8)
...
!     **************************************************************************
! ERROR: CREATES FILE .ADET INDEPENDET OF SELECTION
...
!       OPEN FILE
        ...
        DO I=1,SETTINGS%NE
! ERROR: OUTPUT ONLY FOR GAMMA POINT
          WRITE(NFIL,*) SPECTRUM%E(I)/EV,SPECTRUM%I(1,1,I)
        ENDDO
!       CLOSE FILE
      ...
      IF(OUTPUT%TRAW) THEN
        DO ISPEC=1,SETTINGS%NSPEC
          ...
          NB2=SIM(2)%STATEARR(1,1)%NB
          NOCC=SIM(1)%STATEARR(1,1)%NOCC
          WRITE(NFIL,FMT='(3A14)')'# ENERGY','CROSSSECTION','FINAL BAND'
          DO IFINAL=1,NB2-NOCC
            I=IFINAL+NOCC
! ERROR: OUTPUT ONLY FOR GAMMA POINT
            WRITE(NFIL,*)SPECTRUM%ERAW(1,1,IFINAL)/EV,SPECTRUM%IRAW(1,1,IFINAL),I
          ENDDO
...
! TODO: WRITE OUTPUT SUCH THAT IT CAN EASILY BE READ BY PYTHON
          ...
          IF(OUTPUT%TKMAT) THEN
            ! CALL WRITEMATC8ABS(NB2-NOCC,NB1,OVERLAP%KMAT,'KMAT',IKPT,ISPIN)
            CALL WRITEMATC8(NB2-NOCC,NB1,OVERLAP%KMAT,'KMAT',IKPT,ISPIN)
          ENDIF
