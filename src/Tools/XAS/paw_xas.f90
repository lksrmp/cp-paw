...
!       POLARISATION IN CARTHESIAN COORDINATES
        ...
        IF(TCHK) THEN
          ...
          SPECTRUM%POLXYZ=CMPLX(REALPOLXYZ,KIND=8)
        ENDIF
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE XAS$AMPLITUDE(IKPT,ISPIN,IEMP,POLXYZ,AMPL)  ! MARK: XAS$AMPLITUDE
!     **************************************************************************
...
!     CHECK IF FINAL STATE ORBITAL INDEX IS VALID
      ...
      AMPL=(0.D0,0.D0)
      DO IB=1,STATE%NOCC
        AMPL=AMPL+CONJG(OVERLAP%KMAT(IEMP,IB))*SUM(POLXYZ(:)*OVERLAP%DIPOLE(:,IB))
      ENDDO
      AMPL=SUM(POLXYZ(:)*OVERLAP%DIPOLE(:,IEMP))-AMPL
...
!     **************************************************************************
      ...
      TYPE(STATE_TYPE), POINTER :: STATE
      COMPLEX(8) :: AMPL
      REAL(8), ALLOCATABLE :: ENERGY(:) ! (NB2-NOCC)
      REAL(8), ALLOCATABLE :: SIGMA(:) ! (NB2-NOCC)
      REAL(8) :: EGS,EFI
      REAL(8) :: EV
      REAL(8) :: EMIN,EMAX
      REAL(8) :: E
...
! TODO: CLEANUP
! TODO: CALCULATE AMPLITUDE AS VECTOR AND APPLY POLARIZATION AS LATE AS POSSIBLE
!       COULD AVOID UNNECESSARY CALCULATIONS
      CALL FILEHANDLER$UNIT('PROT',NFIL)
...
!         LOOP OVER SPIN
          DO ISPIN=1,SIM(1)%NSPIN
            STATE=>SIM(2)%STATEARR(IKPT,ISPIN)
            ALLOCATE(ENERGY(STATE%NB-STATE%NOCC))
            ALLOCATE(SIGMA(STATE%NB-STATE%NOCC))
...
! WARNING: WEIGHT OF K POINT AND INVERSION SYMMETRY
              ...
              IF(E.GT.EMAX) EMAX=E
              ENERGY(IFINAL)=E
              SIGMA(IFINAL)=REAL(CONJG(AMPL)*AMPL,KIND=8)
WRITE(NFIL,FMT='(4I10,2F14.8)')ISPEC,IKPT,ISPIN,IFINAL+STATE%NOCC,E/EV,REAL(CONJG(AMPL)*AMPL,KIND=8)
            ENDDO
          ENDDO
        ENDDO
      ENDDO
      EMIN=EMIN-2.D0*EV
      EMAX=EMAX+2.D0*EV
WRITE(NFIL,FMT='(A10,F14.6)')'EMIN',EMIN/EV
WRITE(NFIL,FMT='(A10,F14.6)')'EMAX',EMAX/EV
      NE=INT((EMAX-EMIN)/SETTINGS%DE)+1
      ALLOCATE(X(NE))
      ALLOCATE(Y(NE))
      ALLOCATE(YCONV(NE))
      DO I=1,NE
        X(I)=EMIN+REAL(I-1,KIND=8)*SETTINGS%DE
      ENDDO
      Y(:)=0.D0
      DO I=1,STATE%NB-STATE%NOCC
        CALL XAS$MAPGRID(ENERGY(I),SIGMA(I),NE,Y,EMIN,EMAX,SETTINGS%DE)
      enddo
      CALL LORENTZCONV(NE,X,Y,SETTINGS%GAMMA,YCONV)
      OPEN(UNIT=NFIL,FILE='xas.dat')
      DO I=1,NE
        WRITE(NFIL,FMT='(2F14.6)')X(I)/EV,YCONV(I)
      ENDDO
      CLOSE(NFIL)
