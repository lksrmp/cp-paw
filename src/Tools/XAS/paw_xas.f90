...
!     LOOP OVER K POINTS
      DO IKPT=1,SIM(1)%NKPT
        IF(MOD(IKPT-1,NTASKS).NE.THISTASK-1) CYCLE
...
!        EFFECTED BY SIZE OF SECOND WRITEMATC8 ARGUMENT
          IF(OUTPUT%TOVL) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('OVL')
          ENDIF
          IF(OUTPUT%TOVLPW) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('OVLPW')
          ENDIF
          IF(OUTPUT%TOVLAUG) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('OVLAUG')
          ENDIF
          IF(OUTPUT%TOVLOCC) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('OVLOCC')
          ENDIF
          IF(OUTPUT%TOVLEMP) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('OVLEMP')
          ENDIF
          IF(OUTPUT%TKMAT) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('KMAT')
          ENDIF
          IF(OUTPUT%TDIPOLE) THEN
            ...
            IF(OUTPUT%TBINARY) CALL BINCOMPMAT('DIPOLE')
          ENDIF
...
!         READ WKPT (HAS TO BE THE SAME FOR BOTH SIMULATIONS)
      ...
      DO IS=1,2
        DO IKPT=1,SIM(IS)%NKPT
          DO ISPIN=1,SIM(IS)%NSPIN
            ...
            ENDIF
            CALL MPE$BROADCAST('~',1,SIM(IS)%STATEARR(IKPT,ISPIN)%NB)
            CALL MPE$BROADCAST('~',1,SIM(IS)%STATEARR(IKPT,ISPIN)%NOCC)
            ALLOCATE(SIM(IS)%STATEARR(IKPT,ISPIN)%EIG(SIM(IS)%STATEARR(IKPT,ISPIN)%NB))
            IF(THISTASK.EQ.1) THEN
              READ(NFIL)SIM(IS)%STATEARR(IKPT,ISPIN)%EIG
            ENDIF
            CALL MPE$BROADCAST('~',1,SIM(IS)%STATEARR(IKPT,ISPIN)%EIG)
          ENDDO
        ENDDO
      ENDDO
      ...
      DO IKPT=1,SIM(1)%NKPT
        DO ISPIN=1,SIM(1)%NSPIN
          ...
          ENDIF
          CALL MPE$BROADCAST('~',1,OVERLAP%PW)
          CALL MPE$BROADCAST('~',1,OVERLAP%AUG)
          CALL MPE$BROADCAST('~',1,OVERLAP%DIPOLE)
        ENDDO
      ENDDO
      IF(THISTASK.EQ.1) THEN
