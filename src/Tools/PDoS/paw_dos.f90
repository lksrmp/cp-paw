...
!     ==  WILL BE ATTACHED TO DIFFERENT FILES DURING EXECUTION =================
      ID=+'PDOSOUTRAW'
      CALL FILEHANDLER$SETFILE(ID,.TRUE.,-'.DOSRAW')
      CALL FILEHANDLER$SETSPECIFICATION(ID,'STATUS','REPLACE')
      CALL FILEHANDLER$SETSPECIFICATION(ID,'POSITION','REWIND')
      CALL FILEHANDLER$SETSPECIFICATION(ID,'ACTION','WRITE')
      CALL FILEHANDLER$SETSPECIFICATION(ID,'FORM','UNFORMATTED')
...
!       == CLOSE DOWN ==========================================================
      ...
      IF(MODE.EQ.'SAMPLE') THEN
        CALL ERROR$MSG('RAW OUTPUT FILE NOT SUPPORTED IN SAMPLE MODE')
      ENDIF
      IF(MODE.EQ.'TETRA') THEN
        CALL FILEHANDLER$FILENAME('PDOSOUTRAW',FILE)
        FILE=TRIM(PREFIX)//TRIM(FILE)
        CALL FILEHANDLER$SETFILE('PDOSOUTRAW',.FALSE.,FILE)
        CALL FILEHANDLER$UNIT('PDOSOUTRAW',NFILDOS)
        CALL RAW_OUTPUT(NFILDOS,NBB,NKPT,EIG,NSET,SET)
      ENDIF
                          CALL TRACE$POP
      RETURN
...
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
      SUBROUTINE RAW_OUTPUT(NFILO,NBB,NKPT,EIG,NSET,SET)
!     **************************************************************************
!     **  OUTPUT OF RAW DOS FOR EACH BAND, KPOINT, AND SPIN COMPONENT         **
!     **************************************************************************
      USE STRINGS_MODULE
      USE DOS_WGHT_MODULE, ONLY: EF
      USE NEWSET_MODULE  ,ONLY : NEWSET
      ...
      INTEGER(4),INTENT(IN) :: NKPT ! #(K-POINTS)
      REAL(8)   ,INTENT(IN) :: EIG(NBB,NKPT) ! ONE-PARTICLE ENERGIES
      INTEGER(4),INTENT(IN) :: NSET ! #(DATA SETS)
      REAL(8)   ,INTENT(IN) :: SET(NBB,NKPT,2,NSET) ! MATRIX ELEMENTS
      INTEGER(4)            :: ISET
!     **************************************************************************
                          CALL TRACE$PUSH('RAW_OUTPUT')
      ! NO OUTPUT OF WGHT AND EWGHT AS IT CAN BE RECALCULATED FROM 
      ! NKPT, NBB, EMIN, EIG (WGHT)
      ! NKPT, NBB, EIG, EMIN, EMAX, NE (EWGHT)
      ! REQUIRES SUBSEQUENT CALCULATION OF DOS SIMILAR TO PUTONGRID_TETRAHEDRON
      WRITE(NFILO) NSET,NKPT,NBB
      WRITE(NFILO) EIG,EF
      DO ISET=1,NSET
        ! CHARACTER(32)
        WRITE(NFILO) NEWSET(ISET)%ID
      ENDDO
      WRITE(NFILO) SET
      WRITE(NFILO) +'FINISHED'
                          CALL TRACE$POP
      RETURN
      END SUBROUTINE RAW_OUTPUT
!
!     ...1.........2.........3.........4.........5.........6.........7.........8
